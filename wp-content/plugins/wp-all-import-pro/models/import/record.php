...
class PMXI_Import_Record extends PMXI_Model_Record {
			...
			foreach ( $files as $ind => $path ) {
				...
				if ( ! empty( $filePath ) and @file_exists( $filePath ) ) {
					...
					if ( $this->queue_chunk_number === 1 and $this->processing == 0 ) { // import first cron request
						...
						$file = new PMXI_Chunk( $filePath, array(
						...
						$this->set( array(
							'count'      => $chunks,
							'processing' => 0,
						) )->update(); // set pointer to the first chunk, updates feed elements count and unlock cron process
					...
					if ( ( $this->queue_chunk_number or ! empty( $force_cron_processing ) ) and $this->processing == 0 ) {
						...
						if ( (int) $this->imported + (int) $this->skipped <= (int) $records_to_import ) {
							$file         = new PMXI_Chunk( $filePath, array(
								'element'  => $this->root_element,
								'encoding' => $this->options['encoding'],
								'pointer'  => $this->queue_chunk_number,
							) );
						...
						if ( (int) $records_to_import <= (int) $this->imported + (int) $this->skipped or $records_to_import == $this->queue_chunk_number ) {
							...
							$this->set( array(
								...
								'queue_chunk_number' => 0,
								'registered_on'      => date( 'Y-m-d H:i:s' ),
								// update registered_on to indicated that job has been executed even if no files are going to be imported by the rest of the method
								'iteration'          => ++ $this->iteration,
							) )->update();
		...
		try {
			...
			if ( in_array( $this->options['custom_type'], array( 'taxonomies' ) ) ) {
				...
				if ( ! empty( $this->options['taxonomy_parent'] ) && $this->is_parsing_required( 'is_update_parent' ) ) {
					$taxonomy_parent = XmlImportParser::factory( $xml, $cxpath, $this->options['taxonomy_parent'], $file )->parse( $records );
					$tmp_files[]     = $file;
				} else {
				...
				if ( 'xpath' == $this->options['taxonomy_slug'] && ! empty( $this->options['taxonomy_slug_xpath'] ) && $this->is_parsing_required( 'is_update_slug' ) ) {
					$taxonomy_slug = XmlImportParser::factory( $xml, $cxpath, $this->options['taxonomy_slug_xpath'], $file )->parse( $records );
					$tmp_files[]   = $file;
				} else {
				...
				if ( 'xpath' == $this->options['taxonomy_display_type'] && ! empty( $this->options['taxonomy_display_type_xpath'] ) ) {
					$taxonomy_display_type = XmlImportParser::factory( $xml, $cxpath, $this->options['taxonomy_display_type_xpath'], $file )->parse( $records );
					$tmp_files[]           = $file;
				} else {
			...
			if ( in_array( $this->options['duplicate_indicator'], array( "pid", "title", "slug" ) ) ) {
				...
				if ( ! empty( $this->options[ $this->options['duplicate_indicator'] . '_xpath' ] ) ) {
					$duplicate_indicator_values = XmlImportParser::factory( $xml, $cxpath, $this->options[ $this->options['duplicate_indicator'] . '_xpath' ], $file )->parse( $records );
					$tmp_files[]                = $file;
				} else {
			...
			if ( ! empty( $this->options['is_featured_xpath'] ) ) {
				$is_image_featured = XmlImportParser::factory( $xml, $cxpath, $this->options['is_featured_xpath'], $file )->parse( $records );
				$tmp_files[]       = $file;
			} else {
			...
			if ( $this->is_parsing_required( 'is_update_categories' ) && ! empty( $post_taxonomies ) && ! in_array( $this->options['custom_type'], array(
					'import_users',
					'taxonomies',
					'shop_customer',
					'comments',
				...
				foreach ( $post_taxonomies as $ctx ): if ( "" == $ctx->labels->name or ( class_exists( 'PMWI_Plugin' ) and strpos( $ctx->name, "pa_" ) === 0 and $this->options['custom_type'] == "product" ) ) {
					continue;
				}
			...
			if ( $this->is_parsing_required( 'is_update_custom_fields' ) ) {
				foreach ( $this->options['custom_name'] as $j => $custom_name ) {
					$meta_keys[ $j ] = XmlImportParser::factory( $xml, $cxpath, $custom_name, $file )->parse( $records );
					$tmp_files[]     = $file;
					if ( is_serialized( $this->options['custom_value'][ $j ] ) ) {
						...
						if ( '' != $this->options['custom_value'][ $j ] and ! is_serialized( $this->options['custom_value'][ $j ] ) ) {
							$meta_values[ $j ] = XmlImportParser::factory( $xml, $cxpath, $this->options['custom_value'][ $j ], $file )->parse( $records );
							$tmp_files[]       = $file;
						...
						if ( ! empty( $serialized_values ) and is_array( $serialized_values ) ) {
							...
							foreach ( array_filter( $serialized_values ) as $key => $value ) {
								...
								if ( empty( $k ) or is_numeric( $k ) ) {
									...
									array_push( $serialized_meta_keys, XmlImportParser::factory( $xml, $cxpath, $k, $file )->parse( $records ) );
									$tmp_files[] = $file;
								}
								...
								if ( "" == $v ) {
									...
									array_push( $serialized_meta_values, XmlImportParser::factory( $xml, $cxpath, $v, $file )->parse( $records ) );
									$tmp_files[] = $file;
								}
							}
			...
			if ( in_array( $this->options['custom_type'], [ 'comments', 'woo_reviews' ] ) ) {
				...
				if ( ! empty( $this->options['comment_post'] ) ) {
					$comment_post = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_post'], $file )->parse( $records );
					$tmp_files[]  = $file;
				} else {
				...
				if ( $this->options['custom_type'] == 'woo_reviews' ) {
					...
					if ( ! empty( $this->options['comment_rating'] ) ) {
						$comment_rating = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_rating'], $file )->parse( $records );
						$tmp_files[]    = $file;
					} else {
				...
				if ( ! empty( $this->options['comment_author'] ) ) {
					$comment_author = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_author'], $file )->parse( $records );
					$tmp_files[]    = $file;
				} else {
				...
				if ( ! empty( $this->options['comment_author_email'] ) ) {
					$comment_author_email = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_author_email'], $file )->parse( $records );
					$tmp_files[]          = $file;
				} else {
				...
				if ( ! empty( $this->options['comment_author_url'] ) ) {
					$comment_author_url = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_author_url'], $file )->parse( $records );
					$tmp_files[]        = $file;
				} else {
				...
				if ( ! empty( $this->options['comment_author_IP'] ) ) {
					$comment_author_ip = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_author_IP'], $file )->parse( $records );
					$tmp_files[]       = $file;
				} else {
				...
				if ( ! empty( $this->options['comment_karma'] ) ) {
					$comment_karma = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_karma'], $file )->parse( $records );
					$tmp_files[]   = $file;
				} else {
				...
				if ( "xpath" == $this->options['comment_approved'] ) {
					...
					if ( ! empty( $this->options['comment_approved_xpath'] ) ) {
						$comment_approved = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_approved_xpath'], $file )->parse( $records );
						$tmp_files[]      = $file;
					} else {
				...
				if ( $this->options['custom_type'] == 'woo_reviews' ) {
					if ( "xpath" == $this->options['comment_verified'] ) {
						...
						if ( ! empty( $this->options['comment_verified_xpath'] ) ) {
							$comment_verified = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_verified_xpath'], $file )->parse( $records );
							$tmp_files[]      = $file;
						} else {
				...
				if ( ! empty( $this->options['comment_agent'] ) ) {
					$comment_agent = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_agent'], $file )->parse( $records );
					$tmp_files[]   = $file;
				} else {
				...
				if ( "xpath" == $this->options['comment_type'] && ! empty( $this->options['comment_type_xpath'] ) ) {
					$comment_type = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_type_xpath'], $file )->parse( $records );
					$tmp_files[]  = $file;
				}
				...
				if ( ! empty( $this->options['comment_parent'] ) ) {
					$comment_parent = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_parent'], $file )->parse( $records );
					$tmp_files[]    = $file;
				} else {
						...
						switch ( $this->options['comment_user_id'] ) {
							...
							default:
								if ( ! empty( $this->options['comment_user_id_xpath'] ) ) {
									$comment_user_id = XmlImportParser::factory( $xml, $cxpath, $this->options['comment_user_id_xpath'], $file )->parse( $records );
									$tmp_files[]     = $file;
			...
			foreach ( $this->options['comments'] as $option => $option_xpath ) {
				switch ( $this->options['comments_repeater_mode'] ) {
						...
						if ( ! empty( $this->options['comments_repeater_mode_foreach'] ) ) {
							for ( $k = 0; $k < count( $titles ); $k ++ ) {
								...
								if ( ! empty( $option_xpath ) ) {
									$values      = \XmlImportParser::factory( $xml, $cxpath . $base_xpath, $option_xpath, $file )->parse();
									$tmp_files[] = $file;
								} else {
					...
					default:
						...
						if ( ! empty( $option_xpath ) ) {
							$values      = XmlImportParser::factory( $xml, $cxpath, $option_xpath, $file )->parse( $records );
							$tmp_files[] = $file;
			...
			foreach ( $titles as $i => $void ) {
				...
				if ( in_array( $this->options['custom_type'], [
						'comments',
						'woo_reviews',
					] ) && ( $this->options['update_all_data'] == 'yes' || $this->options['is_update_comment_post_id'] ) ) {
				...
				if ( in_array( $this->options['custom_type'], [
						'comments',
						'woo_reviews',
					] ) && ( $this->options['update_all_data'] == 'yes' || $this->options['is_update_parent'] ) ) {
				...
				if ( $check_for_duplicates ) {
					...
					if ( "manual" != $this->options['duplicate_matching'] ) {
						...
						if ( 'pid' == $this->options['duplicate_indicator'] ) {
							...
							if ( $duplicate_id && ! in_array( $this->options['custom_type'], array(
									'import_users',
									'shop_customer',
									'taxonomies',
									'comments',
					...
					if ( preg_match_all( '%<a\s[^>]*href=(?(?=")"([^"]*)"|(?(?=\')\'([^\']*)\'|([^\s>]*)))%is', $articleData['post_content'], $matches, PREG_PATTERN_ORDER ) ) {
						...
						foreach ( $hrefs as $url ) {
							if ( preg_match( '%^\w+://%i', $url ) ) { // mask only links having protocol
								...
								$list->join( $ruleTable, "$ruleTable.link_id = $linkTable.id" )->join( $destTable, "$destTable.rule_id = $ruleTable.id" )->setColumns( "$linkTable.*" )->getBy( array(
				...
				switch ( $this->options['custom_type'] ) {
						...
						if ( empty( $articleData['ID'] ) or $this->options['update_all_data'] == 'yes' or $this->options['pmsci_is_update_billing_fields'] ) {
							...
							if ( ! empty( $billing_data ) ) {
								foreach ( $billing_data as $key => $value ) {
									if ( // Update all Billing Fields
										( $this->options['pmsci_update_billing_fields_logic'] == 'full_update' ) or // Update only these Billing Fields, leave the rest alone
										( $this->options['pmsci_update_billing_fields_logic'] == 'only' and in_array( $key, $this->options['pmsci_billing_fields_list'] ) ) or // Leave these fields alone, update all other Billing Fields
								...
								case 'copy':
									...
									if ( ! empty( $billing_data ) ) {
										foreach ( $billing_data as $key => $value ) {
											...
											if ( in_array( $shipping_field, array(
												'shipping_phone',
												'shipping_email',
											) ) ) {
												continue;
											}
											if ( // Update all Shipping Fields
												( $this->options['pmsci_update_shipping_fields_logic'] == 'full_update' ) or // Update only these Shipping Fields, leave the rest alone
												( $this->options['pmsci_update_shipping_fields_logic'] == 'only' and in_array( $shipping_field, $this->options['pmsci_shipping_fields_list'] ) ) or // Leave these fields alone, update all other Shipping Fields
					...
					case 'shop_order':
						...
						if ( empty( $this->options['pmwi_order']['notes_add_auto_order_status_notes'] ) ) {
							...
							add_filter( 'woocommerce_new_order_note_data', [
								$this,
						...
						remove_filter( 'woocommerce_new_order_note_data', [
							$this,
							'woocommerce_new_order_note_data',
						] );
				...
				if ( empty( $pid ) ) {
					...
					if ( empty( $articleData['ID'] ) or $this->options['update_all_data'] == 'yes' or ( $this->options['update_all_data'] == 'no' and $this->options['is_update_custom_fields'] ) or ( $this->options['update_all_data'] == 'no' and ! empty( $this->options['is_update_attributes'] ) and $post_type[ $i ] == "product" and class_exists( 'PMWI_Plugin' ) ) ) {
						...
						if ( ! empty( $articleData['ID'] ) ) {
							...
							foreach ( $existing_meta_keys as $cur_meta_key => $cur_meta_val ) {
								...
								if ( in_array( $cur_meta_key, $exclude_fields ) ) {
									continue;
								}
								...
								if ( in_array( $cur_meta_key, $user_fields ) and in_array( $this->options['custom_type'], array(
										'import_users',
										'shop_customer',
									) ) ) {
									continue;
								...
								if ( ! apply_filters( 'pmxi_custom_field_to_delete', $field_to_delete, $pid, $post_type[ $i ], $this->options, $cur_meta_key ) ) {
									continue;
								}
								...
								elseif ( $this->options['update_all_data'] == 'no' and $this->options['is_update_custom_fields'] ) {
									if ( ( $this->options['update_custom_fields_logic'] == "all_except" && ( empty( $this->options['custom_fields_list'] ) or ! in_array( $cur_meta_key, $this->options['custom_fields_list'] ) ) ) || $this->options['update_custom_fields_logic'] == "only" && ! empty( $this->options['custom_fields_list'] ) && in_array( $cur_meta_key, $this->options['custom_fields_list'] ) ) {
										switch ( $this->options['custom_type'] ) {
											case 'import_users':
											case 'shop_customer':
												delete_user_meta( $pid, $cur_meta_key );
					...
					if ( empty( $articleData['ID'] ) or $this->options['update_all_data'] == 'yes' or ( $this->options['update_all_data'] == 'no' and $this->options['is_update_custom_fields'] ) or ( $this->options['update_all_data'] == 'no' and ! empty( $this->options['is_update_attributes'] ) and $post_type[ $i ] == "product" and class_exists( 'PMWI_Plugin' ) ) ) {
						...
						if ( ! empty( $serialized_meta ) ) {
							...
							if ( "no" == $this->options['is_keep_former_posts'] ) {
								...
								if ( empty( $data_to_update ) and $this->options['is_update_custom_fields'] and "only" === $this->options['update_custom_fields_logic'] and ! empty( $this->options['custom_fields_list'] ) and empty( $meta_data['add'][ $meta_type ] ) and empty( $meta_data['update'][ $meta_type ] ) ) {
					...
					// Page Template
					global $wp_version;
					if ( ! empty( $articleData['post_type'] ) && ! in_array( $articleData['post_type'], array(
							'taxonomies',
							'comments',
							'woo_reviews',
							'gf_entries',
						) ) && ( 'page' == $articleData['post_type'] || version_compare( $wp_version, '4.7.0', '>=' ) ) && wp_all_import_is_update_cf( '_wp_page_template', $this->options ) && ( ! empty( $this->options['page_template'] ) || "no" == $this->options['is_multiple_page_template'] ) ) {
					...
					if ( false && ! empty( $comments['content'] ) && ! in_array( $this->options['custom_type'], [
							'comments',
							'woo_reviews',
							'taxonomies',
							'import_users',
											...
											if ( empty( $term ) and ! is_wp_error( $term ) ) {
												...
												if ( empty( $term ) and ! is_wp_error( $term ) ) {
													...
													if ( empty( $this->options['do_not_create_terms'] ) && empty( $term ) && ! is_wp_error( $term ) ) {
														...
														$term      = wp_insert_term( $single_tax['name'], // the term
	...
	protected function executeSQL() {
		...
		$import_entry = ( in_array( $this->options['custom_type'], array(
			'import_users',
			'shop_customer',
		) ) ) ? 'user' : 'post';
	...
	public function deletePostsAjax( $keepPosts = true, $is_deleted_images = 'auto', $is_delete_attachments = 'auto' ) {
		if ( ! $keepPosts ) {
			...
			$missingPosts = $this->wpdb->get_results( $this->wpdb->prepare( $sql, $this->id ) );
	...
	public function deleteRecords( $is_delete_attachments, $is_deleted_images, $ids = array() ) {
		foreach ( $ids as $k => $id ) {
			if ( ! in_array( $this->options['custom_type'], array(
				'import_users',
				'taxonomies',
				'shop_customer',
				'comments',
