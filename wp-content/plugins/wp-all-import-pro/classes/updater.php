	...
	class PMXI_Updater {
		...
		public function show_update_notification( $file, $plugin ) {
			...
			if ( ! empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version, $version_info->new_version, '<' ) ) {
				...
				if ( ! empty( $version_info->php_version ) && version_compare( $this->php_version, $version_info->php_version, '<' ) ) {
					$new_version = "<span class=\"wp-all-import-pro-new-version-notice\">" . sprintf( __( 'WP All Import ' . $version_info->new_version . ' requires PHP %1s or greater, you are using PHP %2s. Please contact your host and tell them to update your server to at least PHP %1s.', 'wp-all-import-pro' ), $version_info->php_version, $this->php_version, $version_info->php_version ) . "</span>";
				} elseif ( empty( $version_info->download_link ) ) {
					if ( $shiny_updates ) {
						$update_msg_classes .= ' post-shiny-updates';
					}
					$new_version = "<span class=\"wp-all-import-pro-new-version-notice\">" . sprintf( __( 'A new version of WP All Import Pro is available. <strong>A valid license is required to enable updates - enter your license key on the <a href="%1$s">Licenses</a> page.</strong>', 'wp-all-import-pro' ), esc_url( admin_url( 'admin.php?page=pmxi-admin-settings' ) ) ) . "</span>";
					$new_version .= "<span class=\"wp-all-import-pro-licence-error-notice\">" . sprintf( __( 'If you don\'t have a license, please see <a href="%1$s" target="_blank">details & pricing</a>. If you do have a license, you can access it at the <a href="%2$s" target="_blank">customer portal</a>.', 'wp-all-import-pro' ), esc_url( 'http://www.wpallimport.com/order-now/' ), esc_url( 'http://www.wpallimport.com/portal/' ) ) . "</span>";
				} else {
					$new_version = "<span class=\"wp-all-import-pro-new-version-notice\">" . sprintf( __( 'There is a new version of WP All Import Pro available. <a target="_blank" class="thickbox" href="%1$s">View version %2$s details</a> or <a href="%3$s" class="update-link">update now</a>.', 'wp-all-import-pro' ), esc_url( $changelog_link ), esc_html( $version_info->new_version ), esc_url( wp_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' ) . $this->name, 'upgrade-plugin_' . $this->name ) ) ) . "</span>";
				}
			}
		...
		function http_request_args( $args, $url ) {
			...
			if ( defined( 'WPALLIMPORT_SIGNATURE' ) ) {
				$args['body'] = array( 'signature' => WPALLIMPORT_SIGNATURE );
			}
