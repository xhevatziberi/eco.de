...
if ( ! function_exists( 'reverse_taxonomies_html' ) ) {
	...
	function reverse_taxonomies_html( $post_taxonomies, $item_id, &$i, $ctx_name = '', $entry = '' ) {
		$childs = array();
		foreach ( $post_taxonomies as $j => $cat ) {
			if ( $cat->parent_id == $item_id and $cat->item_id != $item_id ) {
				$childs[] = $cat;
			}
		}
				...
				foreach ( $childs as $child_cat ) {
					...
					<li id="item_<?php echo $i; ?>" class="dragging">
						<div class="drag-element">
							...
							<input class="widefat xpath_field" type="text"
							       value="<?php echo esc_textarea( $child_cat->xpath ); ?>" />
