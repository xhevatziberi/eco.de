...
class PMXI_Admin_Import extends PMXI_Controller_Admin {
	...
	protected function init() {
		...
		if ( 'PMXI_Admin_Manage' == PMXI_Plugin::getInstance()->getAdminCurrentScreen()->base ) { // prereqisites are not checked when flow control is deligated
			...
			if ( ! $id or $import->getById( $id )->isEmpty() ) { // specified import is not found
				wp_redirect( esc_url_raw( add_query_arg( 'page', 'pmxi-admin-manage', admin_url( 'admin.php' ) ) ) );
				die();
			}
			$this->isWizard = false;
		} else {
	...
	protected function _step_ready( $action ) {
		...
		if ( ! PMXI_Plugin::$session->has_session() or ! empty( PMXI_Plugin::$session->update_previous ) and $update_previous->getById( PMXI_Plugin::$session->update_previous )->isEmpty() or empty( $xml ) or ! @$dom->loadXML( $xml )// FIX: libxml xpath doesn't handle default namespace properly, so remove it upon XML load
		) {
			if ( ! PMXI_Plugin::is_ajax() ) {
				...
				wp_redirect_or_javascript( esc_url_raw( $this->baseUrl ) );
				die();
			}
		}
		...
		if ( ! PMXI_Plugin::$session->get( 'xpath', false ) or empty( $elements ) or ! $elements->length ) {
			...
			wp_redirect_or_javascript( esc_url_raw( add_query_arg( 'action', 'element', $this->baseUrl ) ) );
			die();
		}
		...
		if ( empty( PMXI_Plugin::$session->options ) ) {
			wp_redirect_or_javascript( esc_url_raw( add_query_arg( 'action', 'template', $this->baseUrl ) ) );
			die();
		}
		if ( 'options' == $action ) {
			return true;
		...
		if ( $id ) { // update requested but corresponding import is not found
			if ( $import->getById( $id )->isEmpty() ) {
				if ( ! empty( $_GET['deligate'] ) and $_GET['deligate'] == 'wpallexport' ) {
					wp_redirect( esc_url_raw( add_query_arg( 'pmxi_nt', array(
						'error'   => urlencode( __( 'The import associated with this export has been deleted.', 'wp-all-import-pro' ) ),
						'updated' => urlencode( __( 'Please re-run your export by clicking Run Export on the All Export -> Manage Exports page. Then try your import again.', 'wp-all-import-pro' ) ),
					), remove_query_arg( 'id', $this->baseUrl ) ) ) );
					die();
				} else {
					wp_redirect( esc_url_raw( add_query_arg( 'pmxi_nt', array( 'error' => urlencode( __( 'This import has been deleted.', 'wp-all-import-pro' ) ) ), remove_query_arg( 'id', $this->baseUrl ) ) ) );
					die();
				}
			} else {
		...
		if ( $this->input->post( 'is_submitted' ) and ! $this->errors->get_error_codes() ) {
			...
			if ( ! $this->errors->get_error_codes() ) {
				...
				$session_data = array(
					...
					'csv_paths'          => array( PMXI_Plugin::$csv_path ),
					// ftp import local copies of remote CSV files
					'action'             => 'import',
				...
				if ( empty( $xml ) ) {
					...
					wp_redirect( esc_url_raw( add_query_arg( 'action', 'element', $this->baseUrl ) ) );
					die();
				}
	...
	public function element() {
		...
		$post                         = $this->input->post( array(
			'xpath'          => '',
			'filters_output' => PMXI_Plugin::$session->options['filters_output'] ?? '',
		) );
		$this->data['post']           =& $post;
		...
		if ( $this->input->post( 'is_submitted' ) ) {
			...
			if ( ! $this->errors->get_error_codes() ) {
				...
				PMXI_Plugin::$session->save_data();
				wp_redirect( apply_filters( 'pmxi_element_redirect', esc_url_raw( add_query_arg( 'action', 'template', $this->baseUrl ) ) ) );
				die();
			}
		} else {
	...
	public function evaluate() {
		...
		$post       = $this->input->post( array(
			'xpath'        => '',
			'show_element' => 1,
			'root_element' => PMXI_Plugin::$session->source['root_element'],
			'delimiter'    => '',
			'is_csv'       => 0,
		) );
		...
		exit( json_encode( array(
			...
			'count'          => $this->data['node_list_count'],
	...
	public function evaluate_variations() {
		...
		$post       = $this->input->post( array(
			'xpath'        => '',
			'show_element' => 1,
			'root_element' => ( ! empty( PMXI_Plugin::$session->source['root_element'] ) ) ? PMXI_Plugin::$session->source['root_element'] : '',
			'tagno'        => 0,
			'parent_tagno' => 1,
		) );
		$wp_uploads = wp_upload_dir();
	...
	public function tag( $is_ajax = true ) {
		...
		if ( $is_ajax ) {
			check_ajax_referer( 'wp_all_import_secure', 'security' );
		}
	...
	public function preview() {
		...
		if ( ! $this->errors->get_error_codes() ) {
			...
			// validate root XPath
			try {
				list( $this->data['title'] ) = XmlImportParser::factory( $xml, $xpath, $post['title'], $file )->parse();
				unlink( $file );
			...
			try {
				if ( empty( $xml ) ) {
					...
					if ( ! isset( $this->data['title'] ) || '' == strval( trim( strip_tags( $this->data['title'], '<img><input><textarea><iframe><object><embed>' ) ) ) ) {
						...
						$this->data['title'] = ( $post['is_leave_html'] ) ? html_entity_decode( $this->data['title'] ) : $this->data['title'];
					}
				}
			} catch ( XmlImportException $e ) {
			...
			try {
				if ( empty( $xml ) ) {
					...
					if ( ! isset( $this->data['content'] ) || '' == strval( trim( strip_tags( $this->data['content'], '<img><input><textarea><iframe><object><embed>' ) ) ) ) {
						...
						$this->data['content'] = ( $post['is_leave_html'] ) ? html_entity_decode( $this->data['content'] ) : $this->data['content'];
					}
				}
			} catch ( XmlImportException $e ) {
	...
	public function preview_images() {
		...
		if ( ! $this->errors->get_error_codes() ) {
			...
			foreach ( $local_paths as $key => $path ) {
				...
				$file = new PMXI_Chunk( $path, array(
					'element'  => ( ! empty( $this->data['update_previous']->root_element ) ) ? $this->data['update_previous']->root_element : PMXI_Plugin::$session->source['root_element'],
					'encoding' => $post['import_encoding'],
				) );
			...
			try {
				...
				if ( empty( $xml ) ) {
					...
					if ( empty( $featured_image ) ) {
						...
						list( $this->data['featured_images'] ) = XmlImportParser::factory( $xml, $xpath, $featured_image, $file )->parse();
						unlink( $file );
					}
				}
			} catch ( XmlImportException $e ) {
				...
				if ( empty( $xml ) ) {
					...
					foreach ( $post['tax_logic'] as $ctx => $logic ) {
						if ( $logic == 'hierarchical' and ! empty( $post['tax_hierarchical_logic_entire'][ $ctx ] ) and is_array( $post['tax_hierarchical_xpath'][ $ctx ] ) ) {
							foreach ( $post['tax_hierarchical_xpath'][ $ctx ] as $ctx_path ) {
								if ( empty( $ctx_path ) ) {
									continue;
								}
								list( $d ) = XmlImportParser::factory( $xml, $xpath, $ctx_path, $file )->parse();
								unlink( $file );
	...
	public function template() {
		...
		if ( $this->isWizard ) {
			...
			if ( empty( $default['custom_type'] ) ) {
				$default['custom_type'] = PMXI_Plugin::$session->custom_type;
			}
			if ( empty( $default['taxonomy_type'] ) ) {
				$default['taxonomy_type'] = PMXI_Plugin::$session->taxonomy_type;
			}
			if ( empty( $default['delimiter'] ) ) {
				$default['delimiter'] = PMXI_Plugin::$session->is_csv;
			}
			if ( empty( $default['ftp_host'] ) ) {
				$default['ftp_host'] = PMXI_Plugin::$session->ftp_host;
			}
			if ( empty( $default['ftp_path'] ) ) {
				$default['ftp_path'] = PMXI_Plugin::$session->ftp_path;
			}
			$default['ftp_root'] = PMXI_Plugin::$session->ftp_root;
			if ( empty( $default['ftp_username'] ) ) {
				$default['ftp_username'] = PMXI_Plugin::$session->ftp_username;
			}
			if ( empty( $default['ftp_password'] ) ) {
				$default['ftp_password'] = PMXI_Plugin::$session->ftp_password;
			}
			if ( empty( $default['ftp_private_key'] ) ) {
				$default['ftp_private_key'] = PMXI_Plugin::$session->ftp_private_key;
			}
				...
				if ( ! $this->errors->get_error_codes() ) { // no validation errors found
					...
					if ( $this->isWizard ) {
						...
						if ( ! PMXI_Plugin::$session->get( 'update_previous' ) ) {
							...
							$import->set( ( empty( PMXI_Plugin::$session->source ) ? array() : PMXI_Plugin::$session->source ) + array(
									'xpath'              => PMXI_Plugin::$session->xpath,
		...
		switch ( $post['custom_type'] ) {
			...
			case 'taxonomies':
				...
				if ( ! empty( $meta_keys ) and $meta_keys->count() ) {
					foreach ( $meta_keys as $meta_key ) {
						if ( in_array( $meta_key['meta_key'], $hide_fields ) ) {
							continue;
						}
						$this->data['meta_keys'][] = $meta_key['meta_key'];
					}
					...
					foreach ( $existing_attributes as $key => $existing_attribute ) {
						...
						if ( ! empty( $existing_attribute ) and is_array( $existing_attribute ) ):
							foreach ( $existing_attribute as $attr_key => $value ) {
								if ( strpos( $attr_key, "pa_" ) === false and ! in_array( $attr_key, $this->data['existing_attributes'] ) ) {
									$this->data['existing_attributes'][] = $attr_key;
								}
							}
						endif;
					}
	...
	public function options() {
		...
		if ( $this->isWizard ) {
			...
			if ( empty( $DefaultOptions['custom_type'] ) ) {
				$DefaultOptions['custom_type'] = PMXI_Plugin::$session->custom_type;
			}
			if ( empty( $DefaultOptions['taxonomy_type'] ) ) {
				$DefaultOptions['taxonomy_type'] = PMXI_Plugin::$session->taxonomy_type;
			}
			if ( empty( $DefaultOptions['delimiter'] ) ) {
				$DefaultOptions['delimiter'] = PMXI_Plugin::$session->is_csv;
			}
			if ( empty( $DefaultOptions['ftp_host'] ) ) {
				$DefaultOptions['ftp_host'] = PMXI_Plugin::$session->ftp_host;
			}
			if ( empty( $DefaultOptions['ftp_path'] ) ) {
				$DefaultOptions['ftp_path'] = PMXI_Plugin::$session->ftp_path;
			}
			if ( empty( $DefaultOptions['ftp_root'] ) ) {
				$DefaultOptions['ftp_root'] = PMXI_Plugin::$session->ftp_root;
			}
			if ( empty( $DefaultOptions['ftp_port'] ) ) {
				$DefaultOptions['ftp_port'] = PMXI_Plugin::$session->ftp_port;
			}
			if ( empty( $DefaultOptions['ftp_username'] ) ) {
				$DefaultOptions['ftp_username'] = PMXI_Plugin::$session->ftp_username;
			}
			if ( empty( $DefaultOptions['ftp_password'] ) ) {
				$DefaultOptions['ftp_password'] = PMXI_Plugin::$session->ftp_password;
			}
			if ( empty( $DefaultOptions['ftp_private_key'] ) ) {
				$DefaultOptions['ftp_private_key'] = PMXI_Plugin::$session->ftp_private_key;
			}
			...
			if ( ! $this->isWizard ) {
				...
				switch ( $this->input->post( 'new_type' ) ) {
					...
					case 'ftp':
						...
						if ( $ftp_host !== $this->data['import']['options']['ftp_host'] || $ftp_path !== $this->data['import']['options']['ftp_path'] || $ftp_root !== $this->data['import']['options']['ftp_root'] || $ftp_port !== $this->data['import']['options']['ftp_port'] || $ftp_username !== $this->data['import']['options']['ftp_username'] || $ftp_password !== $this->data['import']['options']['ftp_password'] || $ftp_private_key !== $this->data['import']['options']['ftp_private_key'] ) {
							try {
			...
			if ( ! $this->errors->get_error_codes() ) { // no validation errors found
				...
				if ( $this->isWizard ) {
					...
					if ( ! $this->input->post( 'save_only' ) ) {
						wp_redirect( esc_url_raw( add_query_arg( 'action', 'confirm', $this->baseUrl ) ) );
						die();
					} else {
						$import    = $this->data['update_previous'];
						$is_update = ! $import->isEmpty();
						$import->set( PMXI_Plugin::$session->source + array(
								'xpath'         => PMXI_Plugin::$session->xpath,
								'options'       => PMXI_Plugin::$session->options,
								'count'         => PMXI_Plugin::$session->count,
						...
						wp_redirect( esc_url_raw( add_query_arg( array(
							'page'    => 'pmxi-admin-manage',
							'pmxi_nt' => urlencode( $is_update ? __( 'Import updated', 'wp-all-import-pro' ) : __( 'Import created', 'wp-all-import-pro' ) ),
						), admin_url( 'admin.php' ) ) ) );
						die();
					}
						...
						if ( $history->count() ) {
							foreach ( $history as $file ) {
								...
								if ( ! $history_file->isEmpty() ) {
									$history_file->delete( $history_file_path != $upload_result['filePath'] );
								}
							}
						}
					...
					if ( ! $this->warnings->get_error_codes() ) {
						...
						wp_redirect( esc_url_raw( add_query_arg( array(
							                                         'page'    => 'pmxi-admin-manage',
							                                         'pmxi_nt' => urlencode( __( 'Configuration updated', 'wp-all-import-pro' ) ),
	...
	public function process( $save_history = true ) {
		...
		if ( $ajax_processing ) {
			...
			$logger = function ( $m ) {
				echo "<div class='progress-msg'>" . wp_all_import_filter_html_kses( $m ) . "</div>\n";
				if ( "" != strip_tags( wp_all_import_strip_tags_content( wp_all_import_filter_html_kses( $m ) ) ) ) {
					PMXI_Plugin::$session->log .= "<p>" . strip_tags( wp_all_import_strip_tags_content( wp_all_import_filter_html_kses( $m ) ) ) . "</p>";
					flush();
				}
			};
		}
				...
				foreach ( PMXI_Plugin::$session->local_paths as $key => $path ) {
					...
					if ( $import_done ) {
						foreach ( PMXI_Plugin::$session->local_paths as $chunk_file ) {
							if ( strpos( $chunk_file, "pmxi_chunk_" ) !== false and @file_exists( $chunk_file ) ) {
								wp_all_import_remove_source( $chunk_file, false );
							}
						}
