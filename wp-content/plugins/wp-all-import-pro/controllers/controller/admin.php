...
abstract class PMXI_Controller_Admin extends PMXI_Controller {
	...
	public function __construct() {
		...
		if ( ! empty( $port ) and is_numeric( $port ) ) {
			...
			$url = ( ! empty( $p_url['port'] ) && ! in_array( $p_url['port'], array(
					80,
					443,
				) ) ) ? $url . ':' . $p_url['port'] : $url;
		}
		...
		wp_enqueue_script( 'jquery-nestable', WP_ALL_IMPORT_ROOT_URL . '/static/js/jquery/jquery.mjs.nestedSortable.js', array(
			'jquery',
			'jquery-ui-dialog',
			'jquery-ui-sortable',
			'jquery-ui-draggable',
		...
		wp_enqueue_script( 'jquery-contextmenu', WP_ALL_IMPORT_ROOT_URL . '/static/js/jquery/jquery.ui-contextmenu.min.js', array(
			'jquery',
			'jquery-ui-menu',
		) );
		...
		wp_enqueue_script( 'jquery-plupload', WP_ALL_IMPORT_ROOT_URL . '/static/js/plupload/wplupload.js', array(
			'plupload',
			'jquery',
		) );

		wp_enqueue_script( 'pmxi-admin-script', WP_ALL_IMPORT_ROOT_URL . '/static/js/admin.js', array(
			'jquery',
			'jquery-ui-dialog',
	...
	public function add_admin_scripts() {
		...
		wp_add_inline_script( 'pmxi-admin-script', "var wpai_cm_settings = JSON.parse( decodeURIComponent( '" . esc_js( $cm_settings ) . "' ) );", 'before' );
	}
