...
class AttachmentHandler {
	...
	private function importAttachments() {
		...
		if ( $is_attachments_to_update and ! empty( $attachments_uploads ) and false === $attachments_uploads['error'] and ! empty( self::$attachments[ $this->recordIndex ] ) and ( empty( $articleData['ID'] ) or self::$importRecord->options['update_all_data'] == "yes" or ( self::$importRecord->options['update_all_data'] == "no" and self::$importRecord->options['is_update_attachments'] ) ) ) {
			...
			if ( ! @is_writable( $targetDir ) ) {
				...
				if ( ! is_array( self::$attachments[ $this->recordIndex ] ) ) {
					self::$attachments[ $this->recordIndex ] = array( self::$attachments[ $this->recordIndex ] );
				}
	...
	public function importImages() {
		...
		if ( ! in_array( self::$importRecord->options['custom_type'], array(
			'shop_order',
			'import_users',
			'shop_customer',
		) ) ) {
		...
		if ( $this->isImageToUpdate and ! empty( $this->imageUploadDir ) and false === $this->imageUploadDir['error'] and ( empty( $this->articleData['ID'] ) or self::$importRecord->options['update_all_data'] == "yes" or ( self::$importRecord->options['update_all_data'] == "no" and self::$importRecord->options['is_update_images'] ) ) ) {
			...
			if ( ! empty( self::$images_bundle ) ) {
				...
				foreach ( self::$images_bundle as $this->slug => $this->bundle_data ) {
					...
					if ( ! $is_images_section_enabled && $this->slug == 'pmxi_gallery_image' ) {
						continue;
					}
					...
					if ( ! empty( $featured_images[ $this->recordIndex ] ) ) {
						...
						if ( ! @is_writable( $targetDir ) ) {
							...
							if ( ! empty( $imgs ) ) {
								...
								foreach ( $imgs as $k => $img_url ) {
									...
									if ( $attid && ! is_wp_error( $attid ) ) {
										...
										$update_attachment_meta = array();
										if ( self::$importRecord->options[ $this->option_slug . 'set_image_meta_title' ] and ! empty( $this->img_titles[ $k ] ) ) {
											$update_attachment_meta['post_title'] = trim( $this->img_titles[ $k ] );
										}
										if ( self::$importRecord->options[ $this->option_slug . 'set_image_meta_caption' ] and ! empty( $this->img_captions[ $k ] ) ) {
											$update_attachment_meta['post_excerpt'] = trim( $this->img_captions[ $k ] );
										}
										if ( self::$importRecord->options[ $this->option_slug . 'set_image_meta_description' ] and ! empty( $this->img_descriptions[ $k ] ) ) {
											$update_attachment_meta['post_content'] = trim( $this->img_descriptions[ $k ] );
										}
										if ( self::$importRecord->options[ $this->option_slug . 'set_image_meta_alt' ] and ! empty( $this->img_alts[ $k ] ) ) {
											update_post_meta( $attid, '_wp_attachment_image_alt', trim( $this->img_alts[ $k ] ) );
										}
							...
							if ( ! $success_images and "yes" == self::$importRecord->options[ $this->option_slug . 'create_draft' ] and ! in_array( $this->postType, array(
									'taxonomies',
									'comments',
									'woo_reviews',
								) ) ) {
						...
						// Create entry as Draft if no images are downloaded successfully
						$final_post_type = get_post_type( $this->pid );
						if ( "yes" == self::$importRecord->options[ $this->option_slug . 'create_draft' ] and ! in_array( $this->postType, array(
								'taxonomies',
								'comments',
								'woo_reviews',
							) ) ) {
	...
	public function downloadFile( $url, $image_filepath, $is_cron, $type = 'images' ) {
		...
		if ( ! $downloaded_with_curl && is_wp_error( $response ) ) {
			...
			if ( $type == 'images' ) {
				if ( preg_match( '%\W(svg)$%i', wp_all_import_basename( $image_filepath ) ) || ( $file_info = apply_filters( 'pmxi_getimagesize', @getimagesize( $image_filepath ), $image_filepath ) ) && in_array( $file_info[2], wp_all_import_supported_image_types() ) ) {
					$downloaded = true;
		...
		if ( ! ( ( self::$uploads = wp_upload_dir() ) && false === self::$uploads['error'] ) ) {
			...
			if ( self::$importRecord->options['attachments'] && self::$importRecord->is_parsing_required( 'is_update_attachments' ) ) {
				...
				if ( ! empty( $atchs ) ) {
					...
					if ( $parse_multiple ) {
						foreach ( $atchs as $atch ) {
							$posts_attachments = \XmlImportParser::factory( self::$xml, self::$cxpath, trim( $atch ), $file )->parse( $records );
							$tmp_files[]       = $file;
						...
						self::$attachments = \XmlImportParser::factory( self::$xml, self::$cxpath, self::$importRecord->options['attachments'], $file )->parse( $records );
						$tmp_files[]       = $file;
					}
				}
			} else {
				...
				foreach ( $image_sections as $section ) {
					...
					if ( "no" == self::$importRecord->options[ $section['slug'] . 'download_images' ] ) {
						if ( self::$importRecord->options[ $section['slug'] . 'featured_image' ] ) {
							$featured_images = \XmlImportParser::factory( self::$xml, self::$cxpath, self::$importRecord->options[ $section['slug'] . 'featured_image' ], $file )->parse( $records );
							$tmp_files[]     = $file;
						} else {
						...
						if ( self::$importRecord->options[ $section['slug'] . 'gallery_featured_image' ] ) {
							$featured_images = \XmlImportParser::factory( self::$xml, self::$cxpath, self::$importRecord->options[ $section['slug'] . 'gallery_featured_image' ], $file )->parse( $records );
							$tmp_files[]     = $file;
						} else {
							count( $titles ) and $featured_images = array_fill( 0, count( $titles ), '' );
						}
						...
						if ( self::$importRecord->options[ $section['slug'] . 'download_featured_image' ] ) {
							$featured_images = \XmlImportParser::factory( self::$xml, self::$cxpath, self::$importRecord->options[ $section['slug'] . 'download_featured_image' ], $file )->parse( $records );
							$tmp_files[]     = $file;
						} else {
							count( $titles ) and $featured_images = array_fill( 0, count( $titles ), '' );
						}
					...
					if ( self::$importRecord->options[ $section['slug'] . 'set_image_meta_title' ] ) {
						...
						if ( self::$importRecord->options[ $section['slug'] . 'image_meta_title' ] ) {
							$image_meta_titles = \XmlImportParser::factory( self::$xml, self::$cxpath, self::$importRecord->options[ $section['slug'] . 'image_meta_title' ], $file )->parse( $records );
							$tmp_files[]       = $file;
						} else {
					...
					if ( self::$importRecord->options[ $section['slug'] . 'set_image_meta_caption' ] ) {
						...
						if ( self::$importRecord->options[ $section['slug'] . 'image_meta_caption' ] ) {
							$image_meta_captions = \XmlImportParser::factory( self::$xml, self::$cxpath, self::$importRecord->options[ $section['slug'] . 'image_meta_caption' ], $file )->parse( $records );
							$tmp_files[]         = $file;
						} else {
					...
					if ( self::$importRecord->options[ $section['slug'] . 'set_image_meta_alt' ] ) {
						...
						if ( self::$importRecord->options[ $section['slug'] . 'image_meta_alt' ] ) {
							$image_meta_alts = \XmlImportParser::factory( self::$xml, self::$cxpath, self::$importRecord->options[ $section['slug'] . 'image_meta_alt' ], $file )->parse( $records );
							$tmp_files[]     = $file;
						} else {
					...
					if ( self::$importRecord->options[ $section['slug'] . 'set_image_meta_description' ] ) {
						...
						if ( self::$importRecord->options[ $section['slug'] . 'image_meta_description' ] ) {
							$image_meta_descriptions = \XmlImportParser::factory( self::$xml, self::$cxpath, self::$importRecord->options[ $section['slug'] . 'image_meta_description' ], $file )->parse( $records );
							$tmp_files[]             = $file;
						} else {
					...
					if ( self::$importRecord->options[ $section['slug'] . 'auto_rename_images' ] and ! empty( self::$importRecord->options[ $section['slug'] . 'auto_rename_images_suffix' ] ) && self::$importRecord->options['download_images'] !== 'gallery' ) {
						$auto_rename_images = \XmlImportParser::factory( self::$xml, self::$cxpath, self::$importRecord->options[ $section['slug'] . 'auto_rename_images_suffix' ], $file )->parse( $records );
						$tmp_files[]        = $file;
					} else {
					...
					if ( self::$importRecord->options[ $section['slug'] . 'auto_set_extension' ] and ! empty( self::$importRecord->options[ $section['slug'] . 'new_extension' ] ) && self::$importRecord->options['download_images'] !== 'gallery' ) {
						$auto_extensions = \XmlImportParser::factory( self::$xml, self::$cxpath, self::$importRecord->options[ $section['slug'] . 'new_extension' ], $file )->parse( $records );
						$tmp_files[]     = $file;
					} else {
