<?php if ( ! $is_license_active ): ?>
	<form name="settings" method="post" action="" class="settings">
		...
		<div class="wpallimport-setting-wrapper">
			...
			<table class="form-table">
				...
				<?php foreach ( $addons as $class => $addon ) : if ( ! $addon['active'] ) {
					continue;
				} ?>
					<tr>
						<th scope="row"><label><?php _e( 'WP All Import License Key', 'wp-all-import-pro' ); ?></label>
						</th>
						<td>
							<input type="password" class="regular-text" name="licenses[<?php echo $class; ?>]"
							       value="<?php if ( ! empty( $post['licenses'][ $class ] ) ) {
								       esc_attr_e( PMXI_Plugin::decode( $post['licenses'][ $class ] ) );
							       } ?>" />
							<?php if ( ! empty( $post['licenses'][ $class ] ) ) { ?>

								<?php if ( ! empty( $post['statuses'][ $class ] ) && $post['statuses'][ $class ] == 'valid' ) { ?>
									<div
										class="license-status inline updated"><?php _e( 'Active', 'wp-all-import-pro' ); ?></div>
								<?php } else { ?>
									<input type="submit" class="button-secondary"
									       name="pmxi_license_activate[<?php echo $class; ?>]"
									       value="<?php _e( 'Activate License', 'wp-all-import-pro' ); ?>" />
									<?php if ( ! empty( $_POST['licenses'][ $class ] ) ) { ?>
										<div
											class="license-status inline error"><?php echo $post['statuses'][ $class ]; ?></div>
									<?php } ?>
								<?php } ?>
							...
							<p class="submit-buttons">
								...
								<input type="submit" class="button-primary"
								       value="<?php _e( 'Save License', 'wp-all-import-pro' ); ?>" />
							</p>
						</td>
					</tr>
				<?php endforeach; ?>
	...
	<form name="settings" method="post" action="" class="settings">
		...
		<table class="form-table">
			...
			<tr>
				<th scope="row"><label><?php _e( 'Automatic Scheduling License Key', 'wp-all-import-pro' ); ?></label>
				</th>
				<td>
					<input type="password" class="regular-text" name="scheduling_license"
					       value="<?php if ( ! empty( $post['scheduling_license'] ) ) {
						       esc_attr_e( PMXI_Plugin::decode( $post['scheduling_license'] ) );
					       } ?>" />
					<?php if ( ! empty( $post['scheduling_license'] ) ) { ?>
						...
						<?php } else { ?>
							...
							<?php if ( ! empty( $_POST['scheduling_license'] ) ) { ?>
								<div
									class="license-status inline error"><?php echo $post['scheduling_license_status']; ?></div>
								<input type="hidden" name="scheduling_license_limit"
								       value="<?php echo get_option( 'wpai_wpae_scheduling_license_site_limit', 0 ); ?>">
							<?php } ?>
						<?php } ?>
	...
	<div class="wpallimport-setting-wrapper">
		...
		<?php if ( $templates->total() ): ?>
			...
			<p class="submit-buttons">
				<input type="submit" class="button-primary" name="delete_templates"
				       value="<?php _e( 'Delete Selected', 'wp-all-import-pro' ) ?>" />
				<input type="submit" class="button-primary" name="export_templates"
				       value="<?php _e( 'Export Selected', 'wp-all-import-pro' ) ?>" />
			</p>
		<?php else: ?>
		...
		<p>
			...
			<input type="submit" class="button-primary" name="import_templates"
			       value="<?php _e( 'Import Templates', 'wp-all-import-pro' ) ?>" />
		</p>
		<?php if ( $is_license_active ): ?>
	</div>
<?php endif ?>
...
<form name="settings" method="post" action="" class="settings">
	...
	<table class="form-table">
		...
		<tr>
			...
			<td>
				<input type="text" class="regular-text" name="cron_processing_time_limit"
				       value="<?php echo esc_attr( $post['cron_processing_time_limit'] ); ?>" />
		...
		<tr>
			...
			<td>
				<input type="text" class="regular-text" name="cron_sleep"
				       value="<?php echo esc_attr( $post['cron_sleep'] ); ?>" />
	...
	<table class="form-table">
		...
		<tr>
			...
			<td>
				<fieldset style="padding:0;">
					<legend class="screen-reader-text"><span><?php _e( 'Secure Mode', 'wp-all-import-pro' ); ?></span>
					</legend>
					<input type="hidden" name="secure" value="0" />
					<label for="secure"><input type="checkbox" value="1" id="secure"
					                           name="secure" <?php echo( ( $post['secure'] ) ? 'checked="checked"' : '' ); ?>><?php _e( 'Randomize folder names', 'wp-all-import-pro' ); ?>
					</label>
				</fieldset>
				<p class="description">
					<?php
		...
		<tr>
			...
			<td>
				<input type="text" class="regular-text" name="log_storage"
				       value="<?php echo esc_attr( $post['log_storage'] ); ?>" />
		...
		<tr>
			...
			<td>
				<a class="button-primary wpallimport-clean-up-tmp-files" href="<?php echo esc_url( add_query_arg( array(
					'action'   => 'cleanup',
					'_wpnonce' => wp_create_nonce( '_wpnonce-cleanup_logs' ),
				), $this->baseUrl ) ); ?>"><?php _e( 'Clean Up', 'wp-all-import-pro' ); ?></a>
	...
	<table class="form-table">
		...
		<tr>
			...
			<td>
				<input type="text" class="regular-text" name="large_feed_limit"
				       value="<?php echo esc_attr( $post['large_feed_limit'] ); ?>" />
		...
		<tr>
			...
			<td>
				<fieldset style="padding:0;">
					...
					<label for="pingbacks"><input type="checkbox" value="1" id="pingbacks"
					                              name="pingbacks" <?php echo( ( $post['pingbacks'] ) ? 'checked="checked"' : '' ); ?>><?php _e( 'Enable WP_IMPORTING', 'wp-all-import-pro' ); ?>
					</label>
				</fieldset>
		...
		<tr>
			...
			<td>
				<fieldset style="padding:0;">
					...
					<label for="backups_prompt"><input type="checkbox" value="1" id="backups_prompt"
					                                   name="backups_prompt" <?php echo( ( $post['backups_prompt'] ) ? 'checked="checked"' : '' ); ?>><?php _e( 'Enable "I HAVE BACKUPS" prompt', 'wp-all-import-pro' ); ?>
					</label>
				</fieldset>
		...
		<tr>
			...
			<td>
				<fieldset style="padding:0;">
					...
					<label for="pmxi_auto_retry_import"><input type="checkbox" value="1" id="pmxi_auto_retry_import"
					                                           name="pmxi_auto_retry_import" <?php echo( ! empty( $post['pmxi_auto_retry_import'] ) ? 'checked="checked"' : '' ); ?>><?php _e( 'Enable auto-retry', 'wp_all_import_plugin' ); ?>
					</label>
		...
		<tr>
			...
			<td>
				<fieldset style="padding:0;">
					<input type="text" class="regular-text" name="pmxi_file_download_timeout"
					       value="<?php echo esc_attr( $post['pmxi_file_download_timeout'] ?? 5 ); ?>" />
	...
	<table class="form-table">
		...
		<tr>
			<th scope="row">
				<label><?php _e( 'Force WP All Import to use StreamReader instead of XMLReader to parse all import files', 'wp-all-import-pro' ); ?></label>
			</th>
			<td>
				<fieldset style="padding:0;">
					<input type="hidden" name="force_stream_reader" value="0" />
					<label for="force_stream_reader"><input type="checkbox" value="1" id="force_stream_reader"
					                                        name="force_stream_reader" <?php echo( ( $post['force_stream_reader'] ) ? 'checked="checked"' : '' ); ?>><?php _e( 'Enable Stream Reader', 'wp-all-import-pro' ); ?>
					</label>
				</fieldset>
				...
				<p class="submit-buttons">
					...
					<input type="submit" class="button-primary"
					       value="<?php _e( 'Save Settings', 'wp-all-import-pro' ); ?>" />
				</p>
			</td>
		</tr>
		</tbody>
