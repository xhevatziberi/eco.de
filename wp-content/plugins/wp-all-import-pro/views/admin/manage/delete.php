...
<form method="post">
	<div class="input">
		<div class="input">
			...
			<input type="checkbox" id="is_delete_import" name="is_delete_import" style="position: relative; top: 2px;"
			       value="1" />
			<label for="is_delete_import"><?php _e( 'Delete import', 'wp-all-import-pro' ); ?> </label>
		</div>
		<div class="input">
			<input type="hidden" name="is_delete_posts" value="0" />
			<input type="checkbox" id="is_delete_posts" name="is_delete_posts" class="switcher"
			       style="position: relative; top: 2px;" value="1" />
			<label
				for="is_delete_posts"><?php printf( __( 'Delete %s created by %s', 'wp-all-import-pro' ), strtolower( $cpt_del_name ), empty( $item->friendly_name ) ? $item->name : $item->friendly_name ); ?> </label>
		</div>
		<div class="switcher-target-is_delete_posts" style="padding: 5px 17px;">
			<div class="input">
				...
				<label
					for="is_delete_images"><?php _e( 'Delete associated images from media gallery', 'wp-all-import-pro' ) ?></label>
			</div>
			<div class="input">
				...
				<label
					for="is_delete_attachments"><?php _e( 'Delete associated files from media gallery', 'wp-all-import-pro' ) ?></label>
			</div>
		</div>
		...
		<p class="wp-all-import-sure-to-delete"><?php _e( 'Are you sure you want to delete ', 'wp-all-import-pro' ); ?>
			<span
				class="sure_delete_posts"><?php printf( '<strong>%s %s</strong>', $associated_posts, $cpt_name ); ?></span><span
				class="sure_delete_posts_and_import"> <?php _e( 'and', 'wp-all-import-pro' ); ?> </span><span
				class="sure_delete_import"><?php printf( __( 'the <strong>%s</strong> import', 'wp-all-import-pro' ), empty( $item->friendly_name ) ? $item->name : $item->friendly_name ); ?></span>?
		</p>
	</div>
