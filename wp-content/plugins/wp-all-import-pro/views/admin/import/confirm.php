		...
		<div class="wpallimport-content-section" style="padding: 30px; overflow: hidden;">
			...
			<?php if ( $is_new_import ): ?>
				<form class="confirm <?php echo ! $isWizard ? 'edit' : '' ?>" method="post" style="float:right;">
					...
					<input type="submit" class="rad10"
					       value="<?php _e( 'Confirm & Run Import', 'wp-all-import-pro' ) ?>" />
				</form>
			<?php else: ?>
				<form class="confirm <?php echo ! $isWizard ? 'edit' : '' ?>" method="post" style="float: right;">
					<?php wp_nonce_field( 'confirm', '_wpnonce_confirm' ) ?>
					...
					<div class="input wpallimport-is-continue">
						<div class="input">
							...
							<label
								for="is_continue_yes"><?php _e( 'Continue from the last run', 'wp-all-import-pro' ); ?></label>
						</div>
						<div class="input">
							<input type="radio" name="is_continue" value="no" id="is_continue_no" />
							<label
								for="is_continue_no"><?php _e( 'Run from the beginning', 'wp-all-import-pro' ); ?></label>
						</div>
					</div>
					<input type="submit" class="rad10" value="<?php _e( 'Continue Import', 'wp-all-import-pro' ) ?>"
					       style="margin-left: 0px; float: right;" />
					<!--div class="input" style="margin-top:20px;">
						<a href="<?php echo esc_url( add_query_arg( array( 'id'       => $import->id,
					                                                       'action'   => 'update',
					                                                       'continue' => 'no',
					), $this->baseUrl ) ); ?>" id="entire_run"><?php _e( 'Run entire import from the beginning', 'wp-all-import-pro' ); ?></a>
					</div-->
				</form>
			<?php endif; ?>
		</div>
					...
					<div class="wpallimport-section" style="margin-top: -20px;">
						<div class="wpallimport-content-section">
							<div class="wpallimport-collapsed-header" style="padding-left: 30px;">
								<h3 style="color: #425e99;"><?php _e( 'Import Summary', 'wp-all-import-pro' ); ?> <?php if ( ! $isWizard ): ?>
										<span
											style="color:#000;"><?php printf( __( " - ID: %s - %s", 'wp-all-import-pro' ), $import->id, empty( $import->friendly_name ) ? $import->name : $import->friendly_name ); ?></span><?php endif; ?>
								</h3>
							</div>
							<div class="wpallimport-collapsed-content" style="padding: 15px 25px 25px;">
								...
								<?php else: ?>
									...
									<?php
									$criteria = '';
									if ( 'pid' == $post['duplicate_indicator'] ) {
										$criteria = 'has the same ID';
									}
									...
									if ( 'custom field' == $post['duplicate_indicator'] ) {
										$criteria = 'has Custom Field named "' . $post['custom_duplicate_name'] . '" with value = ' . $post['custom_duplicate_value'];
									}
									?>
