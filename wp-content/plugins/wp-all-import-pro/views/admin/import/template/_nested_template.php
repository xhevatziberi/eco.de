					...
					<td>
							...
							<ul>
								<?php if ( ! empty( $post['nested_files'] ) ): ?>
									...
									foreach ( $nested_files as $key => $file ) {
										?>
										<li rel="<?php echo $key; ?>"><?php echo $file; ?> <a href="javascript:void(0);"
										                                                      class="unmerge"><?php _e( 'remove', 'wp-all-import-pro' ); ?></a>
										</li>
										<?php
									} ?>
								<?php endif; ?>
							</ul>
							<input type="hidden" value="<?php echo esc_attr( $post['nested_files'] ); ?>"
							       name="nested_files" />
						</div>
						...
						<div class="add_nested_file">
							...
							<div class="file-type-options">
								...
								<input type="text" class="regular-text" name="nested_url" value=""
								       style="width:100%; line-height:20px;" placeholder="http(s)://" />
							</div>

							<a rel="parse" href="javascript:void(0);"
							   class="parse"><?php _e( 'Add', 'wp-all-import-pro' ); ?></a>
						</div>
					</td>
				</tr>
			</table>
