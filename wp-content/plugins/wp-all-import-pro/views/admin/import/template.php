...
<form class="wpallimport-template <?php echo ! $this->isWizard ? 'edit' : '' ?> wpallimport-step-3" method="post">
	...
	<?php $visible_sections = apply_filters( 'pmxi_visible_template_sections', array(
		'caption',
		'main',
		'taxonomies',
		'cf',
		...
		<tr>
			<td class="left">
				...
				<?php if ( in_array( 'caption', $visible_sections ) ): ?>
					...
					<div class="wpallimport-collapsed wpallimport-section">
						<div class="wpallimport-content-section"
						     style="overflow: hidden; padding-bottom: 0; margin-top: 0;">
								...
								<div style="padding: 15px 25px 65px;">
									<?php if ( ! in_array( $post_type, [ 'comments', 'woo_reviews' ] ) ): ?>
										<div id="titlediv" style="margin-bottom:20px;">
											<div id="titlewrap">
												<input id="wpallimport-title" class="widefat" type="text" name="title"
												       value="<?php echo esc_attr( $post['title'] ) ?>"
												       placeholder="<?php _e( 'Drag & drop any element on the right to set the title.', 'wp-all-import-pro' ); ?>" />
											</div>
										</div>
									<?php endif; ?>
										...
										<div class="template_input">
											...
											<?php else: ?>
												<input type="text" name="post_excerpt"
												       style="width:100%; line-height: 25px;"
												       value="<?php echo esc_attr( $post['post_excerpt'] ) ?>"
												       placeholder="<?php _e( 'Excerpt', 'wp-all-import-pro' ); ?>" />
											<?php endif; ?>
										</div>
									<?php endif; ?>

									<a class="preview" href="javascript:void(0);"
									   rel="preview"><?php _e( 'Preview', 'wp-all-import-pro' ); ?></a>
								</div>
										...
										<div class="wpallimport-collapsed-content" style="padding: 0;">
											<div class="wpallimport-collapsed-content-inner">
												<div class="input pmxi_option">
													...
													<input type="checkbox" id="is_keep_linebreaks"
													       name="is_keep_linebreaks" class="fix_checkbox"
													       value="1" <?php echo $post['is_keep_linebreaks'] ? 'checked="checked"' : '' ?> />
													<label
														for="is_keep_linebreaks"><?php _e( 'Keep line breaks from file', 'wp-all-import-pro' ) ?></label>
												</div>
												<div class="input pmxi_option">
													<input type="hidden" name="is_leave_html" value="0" />
													<input type="checkbox" id="is_leave_html" name="is_leave_html"
													       class="fix_checkbox"
													       value="1" <?php echo $post['is_leave_html'] ? 'checked="checked"' : '' ?>
													       style="position:relative;" />
													<label
														for="is_leave_html"><?php _e( 'Decode HTML entities with <b>html_entity_decode</b>', 'wp-all-import-pro' ) ?></label>
													<a class="wpallimport-help" href="#help"
													   style="position:relative; top:1px;"
													   title="<?php _e( 'If HTML code is showing up in your posts, use this option. You can also use <br /><br /><i>[html_entity_decode({my/xpath})]</i><br /><br /> or <br /><br /><i>[htmlentities({my/xpath})]</i><br /><br /> or <br /><br /><i>[htmlspecialchars_decode({my/xpath})]</i><br /><br /> to decode or encode HTML in this import file.', 'wp-all-import-pro' ); ?>">?</a>
												</div>
											</div>
										</div>
									</div>
				...
				<input type="hidden" name="type"
				       value="<?php echo ( $post['custom_type'] == 'page' ) ? 'page' : 'post'; ?>" />
				...
				<div class="input wpallimport-section load-template-container"
					...
					<p style="margin: 11px; float: left;">
						...
						<label
							for="save_template_as"><?php _e( 'Save settings as a template', 'wp-all-import-pro' ); ?></label>
					</p>
					<div class="switcher-target-save_template_as" style="float: left; overflow: hidden;">
						<input type="text" name="name"
						       placeholder="<?php _e( 'Template name...', 'wp-all-import-pro' ) ?>"
				...
				<div class="wpallimport-submit-buttons">
					...
					<div style="text-align:center; width:100%;">
						...
						<input type="hidden" name="security"
						       value="<?php echo wp_create_nonce( "wp_all_import_preview" ); ?>" />

						<?php if ( $this->isWizard ): ?>
							<a href="<?php echo esc_url( add_query_arg( 'action', 'element', $this->baseUrl ) ); ?>"
							   class="back rad3"
							   style="float:none;"><?php _e( 'Back to Step 2', 'wp-all-import-pro' ) ?></a>
						<?php else: ?>
							<a href="<?php echo esc_url( remove_query_arg( 'id', remove_query_arg( 'action', $this->baseUrl ) ) ); ?>"
							   class="back rad3"
							   style="float:none;"><?php _e( 'Back to Manage Imports', 'wp-all-import-pro' ) ?></a>
						<?php endif; ?>
						<input type="submit" class="button button-primary button-hero wpallimport-large-button"
						       value="<?php _e( ( $this->isWizard ) ? 'Continue to Step 4' : 'Update Template', 'wp-all-import-pro' ) ?>" />
					</div>
