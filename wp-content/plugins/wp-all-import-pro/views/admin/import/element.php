...
<form class="wpallimport-choose-elements no-enter-submit wpallimport-step-2 wpallimport-wrapper" method="post">
	...
	<div class="wpallimport-content-section wpallimport-console">
		...
		<input type="submit" class="button button-primary button-hero wpallimport-large-button"
		       value="<?php _e( 'Continue to Step 3', 'wp-all-import-pro' ); ?>"
	...
	<div class="wpallimport-content-section" style="padding-bottom:0; max-height: 600px; overflow:scroll; width: 100%;">
		...
		<table class="wpallimport-layout" style="width:100%;">
			<tr>
					...
					<td class="left" style="width: 25%; min-width: unset; border-right: 1px solid #ddd;">
						...
						if ( ! empty( $elements_cloud ) and ! $is_csv ) {
							foreach ( $elements_cloud as $tag => $count ) {
								?>
								<a href="javascript:void(0);" rel="<?php echo $tag; ?>"
								   class="wpallimport-change-root-element <?php if ( PMXI_Plugin::$session->source['root_element'] == $tag ) {
									   echo 'selected';
								   } ?>">
				...
				<?php endif; ?>
				<td class="right" <?php if ( ! $is_csv ){ ?>style="width:75%; padding:0;"
				    <?php } else { ?>style="width:100%; padding:0;"<?php } ?>>
						...
						<div class="input">
							...
							<?php if ( $is_csv !== false ): ?>
								...
								<div class="wpallimport-set-csv-delimiter">
									...
									<input type="button" name="apply_delimiter" class="rad4"
									       value="<?php _e( 'Apply', 'wp-all-import-pro' ); ?>" />
								</div>
	...
	<p class="wpallimport-submit-buttons" style="text-align:center;">
		<a href="<?php echo esc_url( add_query_arg( 'action', 'index', $this->baseUrl ) ); ?>"
		   class="back rad3"><?php _e( 'Back to Step 1', 'wp-all-import-pro' ); ?></a>
		&nbsp;
