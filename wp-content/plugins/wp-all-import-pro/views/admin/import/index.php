				...
				<form method="post" class="wpallimport-choose-file" enctype="multipart/form-data" autocomplete="off">
					...
					<div class="wpallimport-upload-resource-step-one">
						...
						<div class="wpallimport-import-types">
							...
							<?php endif; ?>
							<a class="wpallimport-import-from wpallimport-upload-type <?php echo ( 'upload' == $post['type'] ) ? 'selected' : '' ?>"
							   rel="upload_type" href="javascript:void(0);">
								<span class="wpallimport-icon"></span>
								<span
									class="wpallimport-icon-label"><?php _e( 'Upload a file', 'wp-all-import-pro' ); ?></span>
							</a>
							<a class="wpallimport-import-from wpallimport-url-type <?php echo ( 'url' == $post['type'] || 'ftp' == $post['type'] ) ? 'selected' : '' ?>"
							   rel="url_type" href="javascript:void(0);">
								<span class="wpallimport-icon"></span>
								<span
									class="wpallimport-icon-label"><?php _e( 'Download a file', 'wp-all-import-pro' ); ?></span>
							</a>
							<a class="wpallimport-import-from wpallimport-file-type <?php echo 'file' == $post['type'] ? 'selected' : '' ?>"
							   rel="file_type" href="javascript:void(0);">
								<span class="wpallimport-icon"></span>
								<span
									class="wpallimport-icon-label"><?php _e( 'Use existing file', 'wp-all-import-pro' ); ?></span>
							</a>
						</div>
						...
						<div class="wpallimport-upload-type-container" rel="upload_type">
							<div id="plupload-ui" class="wpallimport-file-type-options">
								<div>
									<input type="hidden" name="filepath" value="<?php echo $post['filepath'] ?>"
									       id="filepath" />
									<a id="select-files" href="javascript:void(0);"
									   <?php if ( empty( $post['filepath'] ) ): ?>style="display:none;"<?php endif; ?> /><?php _e( 'Click here to select file from your computer...', 'wp-all-import-pro' ); ?></a>
									...
									<div id="progress" class="wpallimport-progress"
									     <?php if ( ! empty( $post['filepath'] ) ): ?>style="visibility: visible; display: block;"<?php endif; ?>>
										<?php if ( ! empty( $post['filepath'] ) ): ?>
											<div
												class="wpallimport-upload-process ui-progressbar ui-widget ui-widget-content ui-corner-all"
												id="upload_process" role="progressbar" aria-valuemin="0"
												aria-valuemax="100" aria-valuenow="100">
												<div
													class="ui-progressbar-value ui-widget-header ui-corner-left ui-corner-right"
													style="width: 100%;"></div>
											</div>
										<?php else: ?>
						...
						<div class="wpallimport-upload-type-container" rel="url_type">
							<div class="wpallimport-choose-data-type">
								<a class="wpallimport-download-from rad4 wpallimport-download-file-from-url <?php if ( $post['type'] == 'url' ) {
									echo 'wpallimport-download-from-checked';
								} ?>" rel="url" href="javascript:void(0);">
									<span
										class="wpallimport-download-from-title"><?php _e( 'From URL', 'wp-all-import-pro' ); ?></span>
									<span class="wpallimport-download-from-arrow"></span>
								</a>
								<a class="wpallimport-download-from rad4 wpallimport-download-file-from-ftp <?php if ( $post['type'] == 'ftp' ) {
									echo 'wpallimport-download-from-checked';
								} ?>" rel="ftp" href="javascript:void(0);">
									<span
										class="wpallimport-download-from-title"><?php _e( 'From FTP/SFTP', 'wp-all-import-pro' ); ?></span>
									<span class="wpallimport-download-from-arrow"></span>
								</a>
							</div>
						</div>
						<div class="wpallimport-upload-type-container" rel="file_type">
							...
							<div class="wpallimport-file-type-options">
								...
								$local_files = array_merge( PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.xml', $glob_flags ), PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.gz', $glob_flags ), PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.zip', $glob_flags ), PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.gzip', $glob_flags ), PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.csv', $glob_flags ), PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.dat', $glob_flags ), PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.psv', $glob_flags ), PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.json', $glob_flags ), PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.txt', $glob_flags ), PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.sql', $glob_flags ), PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.xls', $glob_flags ), PMXI_Helper::safe_glob( $upload_dir['basedir'] . $files_directory . '*.xlsx', $glob_flags ) );
								sort( $local_files );
								$sizes = array();
								...
								<select id="file_selector">
									<option
										value=""><?php _e( 'Select a previously uploaded file', 'wp-all-import-pro' ); ?></option>
									<?php foreach ( $local_files as $file ) : ?>
										<option value="<?php echo $file; ?>"
										        <?php if ( $file == esc_attr( $post['file'] ) ): ?>selected="selected"<?php endif; ?>><?php echo basename( $file ); ?></option>
									<?php endforeach; ?>
								</select>
							...
							<div class="wpallimport-download-resource-step-two">
								<div class="wpallimport-download-resource wpallimport-download-resource-step-two-url">
									<div class="wpallimport-file-type-options">
										...
										<input type="text" class="regular-text" name="url"
										       value="<?php echo ( ! empty( $post['url'] ) ) ? esc_attr( $post['url'] ) : ''; ?>"
										       placeholder="<?php _e( 'Enter a web address to download the file from...', 'wp-all-import-pro' ); ?>" />
										<a class="wpallimport-download-from-url rad4"
										   href="javascript:void(0);"><?php _e( 'Download', 'wp-all-import-pro' ); ?></a>
										<span class="img_preloader"
										      style="top:0; left: 5px; visibility: hidden; display: inline;"></span>
									</div>
									...
									<input type="hidden" name="downloaded"
									       value="<?php echo esc_attr( $post['downloaded'] ); ?>" />
									<input type="hidden" name="template"
									       value="<?php echo esc_attr( $post['template'] ); ?>" />
									<input type="hidden" name="bundle_xpath"
									       value="<?php echo esc_attr( $post['bundle_xpath'] ?? '' ); ?>" />
								</div>
								<div class="wpallimport-download-resource wpallimport-download-resource-step-two-ftp">
									...
									<div class="wpallimport-file-type-options">
										...
										<input type="text" class="regular-text" name="ftp_host"
										       value="<?php echo ( ! empty( $post['ftp_host'] ) ) ? esc_attr( $post['ftp_host'] ) : ''; ?>"
										       placeholder="<?php _e( 'FTP server address', 'wp-all-import-pro' ); ?>" />
										<a class="wpallimport-help" href="#help" style="position: relative; top: -2px;"
										   title="<?php _e( 'The server address of your FTP/SFTP server. This can be an IP address or domain name. You do not need to include the connection protocol. For example, files.example.com or 127.0.0.1', 'wp-all-import-pro' ); ?>">?</a>
									</div>
									<div class="wpallimport-file-type-options">
										<span class="wpallimport-input-icon wpallimport-ftp-port-icon"></span>
										<input type="text" class="regular-text" name="ftp_port"
										       value="<?php echo ( ! empty( $post['ftp_port'] ) ) ? esc_attr( $post['ftp_port'] ) : ''; ?>"
										       placeholder="<?php _e( 'FTP port', 'wp-all-import-pro' ); ?>" />
										<a class="wpallimport-help" href="#help" style="position: relative; top: -2px;"
										   title="<?php _e( 'The port that your server uses. FTP usually uses port 21, SFTP usually uses port 22', 'wp-all-import-pro' ); ?>">?</a>
									</div>
									<div class="wpallimport-file-type-options">
										<span class="wpallimport-input-icon wpallimport-ftp-username-icon"></span>
										<input type="text" class="regular-text" name="ftp_username"
										       value="<?php echo ( ! empty( $post['ftp_username'] ) ) ? esc_attr( $post['ftp_username'] ) : ''; ?>"
										       placeholder="<?php _e( 'FTP username', 'wp-all-import-pro' ); ?>" />
										<a class="wpallimport-help" href="#help" style="position: relative; top: -2px;"
										   title="<?php _e( 'If you don\'t know your FTP/SFTP username, contact the host of the server.', 'wp-all-import-pro' ); ?>">?</a>
									</div>
									<div class="wpallimport-file-type-options">
										<span class="wpallimport-input-icon wpallimport-ftp-password-icon"></span>
										<input type="text" class="regular-text" name="ftp_password"
										       value="<?php echo ( ! empty( $post['ftp_password'] ) ) ? esc_attr( $post['ftp_password'] ) : ''; ?>"
										       placeholder="<?php _e( 'FTP password', 'wp-all-import-pro' ); ?>" />
										<a class="wpallimport-help" href="#help" style="position: relative; top: -2px;"
										   title="<?php _e( 'These passwords are stored in plaintext in your WordPress database. Ideally, the user account should only have read access to the files that you are importing.
<br/><br/>Even if the password is correct, sometimes your host will require SFTP connections to use an SSH key and will deny connection attempts using passwords. If you\'re unable to login and you are sure the password is correct, contact the host of the server.', 'wp-all-import-pro' ); ?>">?</a>
									...
									<div class="wpallimport-file-type-options">
										...
										<textarea class="wpai-ftp-text-area" name="ftp_private_key"
										          value="<?php echo ( ! empty( $post['ftp_private_key'] ) ) ? esc_attr( $post['ftp_private_key'] ) : ''; ?>"
										          placeholder="<?php _e( 'SFTP Private Key', 'wp-all-import-pro' ); ?>"></textarea>
										<a class="wpallimport-help" id="wpai-ftp-text-area-help" href="#help"
										   style="position: relative; top: -2px;"
										   title="<?php _e( 'If you don\'t know if you need an SFTP Private Key, contact the host of the server.', 'wp-all-import-pro' ); ?>">?</a>
									</div>
							...
							<div class="wpallimport-upload-resource-step-two">
								...
								<div class="wpallimport-choose-post-type">
									...
									<input type="hidden" name="wizard_type"
									       value="<?php echo $post['wizard_type']; ?>" />

									<h2 style="margin-top:0;"><?php _e( 'Import data from this import file into...', 'wp-all-import-pro' ); ?></h2>

									<div class="wpallimport-choose-data-type">
										<a class="wpallimport-import-to rad4 wpallimport-to-new-items <?php if ( $post['wizard_type'] == 'new' ) {
											echo 'wpallimport-import-to-checked';
										} ?>" rel="new" href="javascript:void(0);">
											<span
												class="wpallimport-import-to-title"><?php _e( 'New Items', 'wp-all-import-pro' ); ?></span>
											<span class="wpallimport-import-to-arrow"></span>
										</a>
										<a class="wpallimport-import-to rad4 wpallimport-to-existing-items <?php if ( $post['wizard_type'] == 'matching' ) {
											echo 'wpallimport-import-to-checked';
										} ?>" rel="matching" href="javascript:void(0);">
											<span
												class="wpallimport-import-to-title"><?php _e( 'Existing Items', 'wp-all-import-pro' ); ?></span>
											<span class="wpallimport-import-to-arrow"></span>
										</a>
									</div>
									...
									$hidden_post_types = get_post_types( array(
										'_builtin' => false,
										'show_ui'  => false,
									), 'objects' );
									...
									<div class="wpallimport-choose-import-direction">
										<div class="wpallimport-extra-text-left">
											<div
												class="wpallimport-new-records"><?php _e( 'Create new', 'wp-all-import-pro' ); ?></div>
											<div
												class="wpallimport-existing-records"><?php _e( 'Import to existing', 'wp-all-import-pro' ); ?></div>
										</div>
										<div class="wpallimport-extra-text-right">
											<div
												class="wpallimport-new-records"><?php _e( 'for each record in my data file.', 'wp-all-import-pro' ); ?>
												<a class="wpallimport-help" href="#help"
												   style="position: relative; top: -2px;"
												   title="<?php _e( 'The New Items option is commonly used to import new posts or products to your site without touching the existing records.<br/><br/>If the import is later run again with modified data, WP All Import will only update/remove posts created by this import.', 'wp-all-import-pro' ); ?>">?</a>
											</div>
											<div
												class="wpallimport-existing-records"><?php _e( 'and update some or all of their data.', 'wp-all-import-pro' ); ?>
												<a class="wpallimport-help" href="#help"
												   style="position: relative; top: -2px;"
												   title="<?php _e( 'The Existing Items option is commonly used to update existing products with new stock quantities while leaving all their other data alone, update properties on your site with new pricing, etc. <br/><br/> In Step 4, you will map the records in your file to the existing items on your site and specify which data points will be updated and which will be left alone.', 'wp-all-import-pro' ); ?>">?</a>
											</div>
										</div>
										<select name="custom_type_selector" id="custom_type_selector"
										        class="wpallimport-post-types">
											...
											foreach ( $order_arr as $key => $post_name ) {
												if ( array_key_exists( $post_name, $all_posts ) ) {
													...
													<option value="<?php echo $cpt; ?>"
													        data-imagesrc="<?php echo $img_to_echo; ?>"
													        <?php if ( $custom_selected_post === true ): ?>selected="selected"
													        <?php else: if ( $cpt == $post['custom_type'] ): ?>selected="selected"<?php endif;
													endif; ?>><?php echo $cpt_label; ?></option>
													<?php
												}
											}
											?>
										...
										<div class="taxonomy_to_import_wrapper">
											<input type="hidden" name="taxonomy_type"
											       value="<?php echo $post['taxonomy_type']; ?>">
											<h2 style="margin: 30px 0 -10px 0;"><?php _e( 'Select taxonomy to import into...', 'wp-all-import-pro' ); ?>
												<a class="wpallimport-help" href="#help"
												   style="position: relative; top: -2px;"
												   title="Hover over each entry to view the taxonomy slug.">?</a></h2>
											<select id="taxonomy_to_import">
												<option
													value=""><?php _e( 'Select Taxonomy', 'wp-all-import-pro' ); ?></option>
												...
												<?php foreach ( $options as $slug => $name ): ?>
													<option value="<?php echo $slug; ?>"
													        <?php if ( $post['taxonomy_type'] == $slug ): ?>selected="selected"<?php endif; ?>><?php echo $name; ?></option>
												<?php endforeach; ?>
											</select>
										</div>
										<?php if ( ! class_exists( 'PMUI_Plugin' ) ): ?>
											<div class="wpallimport-upgrade-notice" rel="import_users">
												...
												<a href="https://www.wpallimport.com/checkout/?edd_action=add_to_cart&download_id=5839963&edd_options%5Bprice_id%5D=1"
												   target="_blank"
												   class="upgrade_link"><?php _e( 'Purchase the User Add-On', 'wp-all-import-pro' ); ?></a>
											</div>
										<?php endif; ?>
										<?php if ( class_exists( 'WooCommerce' ) && ! class_exists( 'PMWI_Plugin' ) ): ?>
											<div class="wpallimport-upgrade-notice" rel="product">
												...
												<a href="https://www.wpallimport.com/checkout/?edd_action=add_to_cart&download_id=5839961&edd_options%5Bprice_id%5D=1"
												   target="_blank"
												   class="upgrade_link"><?php _e( 'Purchase the WooCommerce Add-On Pro', 'wp-all-import-pro' ); ?></a>
											</div>
										<?php endif; ?>
										<?php if ( class_exists( 'GFForms' ) && ! class_exists( 'PMGI_Plugin' ) ): ?>
											<div class="wpallimport-upgrade-notice" rel="gf_entries">
												...
												<a href="https://www.wpallimport.com/checkout/?edd_action=add_to_cart&download_id=5839927&edd_options%5Bprice_id%5D=1"
												   target="_blank"
												   class="upgrade_link"><?php _e( 'Purchase the Gravity Forms Add-On Pro', 'wp-all-import-pro' ); ?></a>
											</div>
										<?php endif; ?>
					...
					<div
						class="rad4 first-step-errors error-file-validation" <?php if ( ! empty( $upload_validation ) ): ?> style="display:block;" <?php endif; ?>>
						<div class="wpallimport-notify-wrapper">
							<div class="error-headers exclamation">
					...
					<p class="wpallimport-submit-buttons">
						...
						<a href="javascript:void(0);" class="back rad3 auto-generate-template"
						   style="float:none; background: #e4e6e6; padding: 0 50px;"><?php _e( 'Skip to Step 4', 'wp-all-import-pro' ); ?></a>
						<input type="submit" class="button button-primary button-hero wpallimport-large-button"
						       value="<?php _e( 'Continue to Step 2', 'wp-all-import-pro' ) ?>" id="advanced_upload" />
					</p>

					<table>
						<tr>
							<td class="wpallimport-note"></td>
						</tr>
					</table>
				</form>
