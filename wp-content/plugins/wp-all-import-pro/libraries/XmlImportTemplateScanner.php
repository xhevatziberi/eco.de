		...
		while ( ( $ch = $input->peek() ) !== false ) {
			if ( $ch == '}' && ( strlen( $accum ) == 0 || $accum[ strlen( $accum ) - 1 ] != "\\" ) ) {
				...
				if ( $insideLang ) {
					if ( $this->isLangBegin ) {
						return array(
							new XmlImportToken( XmlImportToken::KIND_PRINT ),
							new XmlImportToken( XmlImportToken::KIND_XPATH, $accum ),
						);
					} else {
		...
		if ( in_array( strtoupper( trim( $accum ) ), $this->keywords ) ) {
			...
			if ( $this->isLangBegin ) {
				...
				if ( function_exists( $accum ) or in_array( $accum, array( 'array' ) ) ) {
					return array(
						new XmlImportToken( XmlImportToken::KIND_PRINT ),
						new XmlImportToken( XmlImportToken::KIND_FUNCTION, $accum ),
					);
				} else {
		...
		while ( ( $ch = $input->peek() ) !== false ) {
			if ( $ch == '"' && ( strlen( $accum ) == 0 || $accum[ strlen( $accum ) - 1 ] != "\\" ) ) {
				...
				if ( $this->isLangBegin ) {
					...
					return array(
						new XmlImportToken( XmlImportToken::KIND_PRINT ),
						new XmlImportToken( XmlImportToken::KIND_STRING, $accum ),
					);
				} else {
	...
	private function scanNumber( XmlImportReaderInterface $input ) {
		...
		if ( $isInt ) {
			if ( $this->isLangBegin ) {
				...
				return array(
					new XmlImportToken( XmlImportToken::KIND_PRINT ),
					new XmlImportToken( XmlImportToken::KIND_INT, intval( $accum ) ),
				);
			} else {
			...
			if ( $this->isLangBegin ) {
				...
				return array(
					new XmlImportToken( XmlImportToken::KIND_PRINT ),
					new XmlImportToken( XmlImportToken::KIND_FLOAT, floatval( $accum ) ),
				);
			} else {
