...
function pmxi_admin_notices() {
	...
	if ( class_exists( 'PMWI_Plugin' ) and ( defined( 'PMWI_VERSION' ) and version_compare( PMWI_VERSION, '4.0.0-beta-1.1' ) < 0 and PMWI_EDITION == 'paid' or defined( 'PMWI_FREE_VERSION' ) and version_compare( PMWI_FREE_VERSION, '1.2.1' ) <= 0 and PMWI_EDITION == 'free' ) ) {
		?>
		<div class="error"><p>
				<?php printf( __( '<b>%s Plugin</b>: Please update your WP All Import WooCommerce add-on to the latest version (4.0.0+)', 'wp-all-import-pro' ), PMWI_Plugin::getInstance()->getName() ) ?>
			</p></div>
		<?php
		/*
	...
	if ( class_exists( 'PMAI_Plugin' ) and defined( 'PMAI_VERSION' ) and version_compare( PMAI_VERSION, '3.0.0-beta1' ) < 0 and PMAI_EDITION == 'paid' ) {
		?>
		<div class="error"><p>
				<?php printf( __( '<b>%s Plugin</b>: Please update your WP All Import ACF add-on to the latest version', 'wp-all-import-pro' ), PMAI_Plugin::getInstance()->getName() ) ?>
			</p></div>
		<?php
	...
	if ( class_exists( 'PMLCA_Plugin' ) and defined( 'PMLCA_VERSION' ) and version_compare( PMLCA_VERSION, '1.0.0-beta1' ) < 0 and PMLCA_EDITION == 'paid' ) {
		?>
		<div class="error"><p>
				<?php printf( __( '<b>%s Plugin</b>: Please update your WP All Import Linkcloak add-on to the latest version', 'wp-all-import-pro' ), PMLCA_Plugin::getInstance()->getName() ) ?>
			</p></div>
		<?php
	...
	if ( class_exists( 'PMUI_Plugin' ) and defined( 'PMUI_VERSION' ) and version_compare( PMUI_VERSION, '1.0.0-beta1' ) < 0 and PMUI_EDITION == 'paid' ) {
		?>
		<div class="error"><p>
				<?php printf( __( '<b>%s Plugin</b>: Please update your WP All Import User add-on to the latest version', 'wp-all-import-pro' ), PMUI_Plugin::getInstance()->getName() ) ?>
			</p></div>
		<?php
	...
	if ( class_exists( 'PMLI_Plugin' ) and defined( 'PMLI_VERSION' ) and version_compare( PMLI_VERSION, '1.0.0-beta1' ) < 0 and PMLI_EDITION == 'paid' ) {
		?>
		<div class="error"><p>
				<?php printf( __( '<b>%s Plugin</b>: The WPML Add-On Plugin is no longer compatible with this version of WP All Import - please contact support@wpallimport.com and we will supply the latest version of WP All Import that is compatible with the WPML Add-On.', 'wp-all-import-pro' ), PMLI_Plugin::getInstance()->getName() ) ?>
			</p></div>
		<?php
