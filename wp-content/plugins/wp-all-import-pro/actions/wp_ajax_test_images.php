...
function pmxi_wp_ajax_test_images() {
	...
	if ( ! check_ajax_referer( 'wp_all_import_secure', 'security', false ) ) {
		exit( json_encode( array( 'result'      => array(),
		                          'failed_msgs' => array( __( 'Security check', 'wp-all-import-pro' ) ),
		) ) );
	}
					...
					foreach ( $post['imgs'] as $img ) {
						...
						if ( ( is_wp_error( $request ) or $request === false ) and ! @file_put_contents( $image_filepath, @file_get_contents( $img, false, $get_ctx ) ) ) {
							...
							if ( strpos( $fileContent, '<svg' ) !== false ) {
								...
								$success_images ++;
							} elseif ( ! ( $image_info = apply_filters( 'pmxi_getimagesize', @getimagesize( $image_filepath ), $image_filepath ) ) || ! in_array( $image_info[2], wp_all_import_supported_image_types() ) ) {
