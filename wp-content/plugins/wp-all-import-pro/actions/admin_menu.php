...
function pmxi_admin_menu() {
	...
	if ( current_user_can( PMXI_Plugin::$capabilities ) ) { // admin management options
		...
		add_menu_page( __( 'WP All Import', 'wp-all-import-pro' ), __( 'All Import', 'wp-all-import-pro' ), PMXI_Plugin::$capabilities, 'pmxi-admin-home', array(
			PMXI_Plugin::getInstance(),
			'adminDispatcher',
		), 'data:image/svg+xml;base64,' . $icon_base64, 112 );
		// workaround to rename 1st option to `Home`
		$submenu['pmxi-admin-home'] = array();

		foreach ( $wpai_menu as $key => $value ) {
			add_submenu_page( 'pmxi-admin-home', $value[1], $value[1], PMXI_Plugin::$capabilities, $value[0], array(
				PMXI_Plugin::getInstance(),
				'adminDispatcher',
			) );
		}
