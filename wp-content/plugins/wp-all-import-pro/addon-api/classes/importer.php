...
class PMXI_Addon_Importer {
	...
	public function defaults(
		string $type, string $subtype = null
	) {
		$addon_options = [];
		$options       = [];
	...
	public function getImportFields( $type, $subtype, $groups, $data ) {
		...
		$fields = array_filter( $this->addon->fields( $type, $subtype ), fn( $field ) => isset( $data[ $field['key'] ] ) );

		// Filter out fields from disabled groups
		$fields = array_filter( $fields, fn( $field ) => in_array( $field['group'], $groups ) );
	...
	public function simplify( array $importData, array $parsedData ) {
		...
		$view    = PMXI_Addon_View::create( $this->addon->slug, $type, $subtype );
		...
		foreach ( $fields as $field_index => $field ) {
			...
			if ( isset( $switchers[ $field_slug ] ) && isset( $multiples[ $field_slug ] ) && $switchers[ $field_slug ] == 'yes' ) {
			...
			$data[ $field_slug ] = $field_instance->beforeImport( $post_id, $field_value, $importData, $importData['logger'], $import_options[ $field_slug ] ?? [] );
				...
				foreach ( $data[ $field_slug ] as $rowIndex => $row ) {
					foreach ( $field['subfields'] as $subfield ) {
						...
						if ( isset( $row[ $subfield_slug ] ) ) {
							$data[ $field_slug ][ $rowIndex ][ $subfield_slug ] = $this->castValue( $subfield, $row[ $subfield_slug ], $post_id, $type );
						}
					}
				}
			}
	...
	public function canUpdateField( $field, $import_options ) {
		...
		if ( $rules['update_logic'] === "only" && ! empty( $rules['fields_list'] ) && is_array( $rules['fields_list'] ) && in_array( $field['key'], $rules['fields_list'] ) ) {
			return true;
		}

		if ( $rules['update_logic'] === "all_except" && ( empty( $rules['fields_list'] ) || ! in_array( $field['key'], $rules['fields_list'] ) ) ) {
			return true;
		}
