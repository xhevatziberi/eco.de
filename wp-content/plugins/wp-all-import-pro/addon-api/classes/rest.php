...
class PMXI_Addon_Rest {
	...
	public function validateRequest( \WP_REST_Request $request ) {
		...
		foreach ( $params as $param ) {
			if ( ! $request->get_param( $param ) ) {
				return new \WP_Error( 'missing_param', __( sprintf( "Missing %s parameter", $param ), 'wp-all-import-pro' ), [ 'status' => 400 ] );
			}
		}

		if ( ! PMXI_Addon_Manager::get_addon( $addon ) ) {
			return new \WP_Error( 'addon_not_found', __( 'Addon not found', 'wp-all-import-pro' ), [ 'status' => 400 ] );
		}
	...
	public function fields( \WP_REST_Request $request ) {
		...
		$html = PMXI_Addon_View::create( $addonId, $type, $subtype, $groupId )->getFieldsHtml();
