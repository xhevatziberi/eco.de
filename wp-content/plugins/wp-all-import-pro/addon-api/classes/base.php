...
abstract class PMXI_Addon_Base {
	...
	abstract public static function import(
		int $id, array $fields, array $values, \PMXI_Import_Record $record, array $post, $logger
	);
	...
	public function getImportFields( $type, $subtype, $groups, $data ) {
		...
		$fields = array_filter( $this->fields( $type, $subtype ), fn( $field ) => isset( $data[ $field['key'] ] ) );
...
trait HasRegistration {
	...
	public function registerAsAddon() {
		add_filter( 'pmxi_new_addons', function ( $addons ) {
...
trait HasError {
	...
	public function getMissingDependencyError( $pluginName, $pluginUrl ) {
		return new \WP_Error( 'missing_dependency', __( sprintf( "<b>%s Plugin</b>: <a target=\"_blank\" href=\"%s\">%s</a> must be installed", $this->name(), $pluginUrl, $pluginName ), 'wp-all-import-pro' ) );
	}
}
