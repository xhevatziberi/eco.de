...
<div class="pmxi-repeater-mode pmxi-switcher">
	<div class="pmxi-switcher-radio-group">
		<label class="pmxi-switcher-radio-item">
			<input type="radio" id="<?php echo $switcher_id; ?>_fixed" class="switcher"
			       name="<?php echo esc_attr( $html_name ); ?>[mode]"
			       value="fixed" <?php echo ( $current_mode == 'fixed' ) ? 'checked="checked"' : ''; ?> />
			<span>Fixed</span>
		</label>

		<label class="pmxi-switcher-radio-item">
			<input type="radio" id="<?php echo $switcher_id; ?>_xml" class="switcher"
			       name="<?php echo esc_attr( $html_name ); ?>[mode]"
			       value="variable-xml" <?php echo ( $current_mode == 'variable-xml' ) ? 'checked="checked"' : ''; ?> />
			<span>Variable (XML)</span>
		</label>

		<label class="pmxi-switcher-radio-item">
			<input type="radio" id="<?php echo $switcher_id; ?>_csv" class="switcher"
			       name="<?php echo esc_attr( $html_name ); ?>[mode]"
			       value="variable-csv" <?php echo ( $current_mode == 'variable-csv' ) ? 'checked="checked"' : ''; ?> />
	...
	<div class="pmxi-switcher-target switcher-target-<?php echo $switcher_id; ?>_xml">
		<p>
			...
			<a href="http://www.wpallimport.com/documentation/jetengine/repeater-fields/"
			   target="_blank"><?php _e( '(documentation)', 'wp_all_import_acf_add_on' ); ?></a>
		</p>
	</div>
...
<div class="input">
	...
	<input type="checkbox" id="<?php echo $switcher_id . '_ignore_blanks'; ?>" value="1"
	       name="<?php echo esc_attr( $html_name ); ?>[ignore_blanks]" <?php echo ( ! empty( $field_value['ignore_blanks'] ) ) ? 'checked="checked"' : ''; ?>>
	<label for="<?php echo $switcher_id . '_ignore_blanks'; ?>">Ignore blank fields</label>
	<a href="#help" class="wpallimport-help" style="top:0;"
	   title="<?php _e( 'If the value of the element or column in your file is blank, it will be ignored. Use this option when some records in your file have a different number of repeating elements than others.', 'wp_all_import_acf_add_on' ) ?>">?</a>
</div>
