...
class PMXI_Addon_Map_Field extends PMXI_Addon_Map_Provider_Field {
	...
	public function getCachedData( $address = null, $lat = null, $lng = null ) {
		...
		if ( $address ) {
			$query = $wpdb->prepare( "SELECT * FROM $table WHERE address = %s LIMIT 1", $address );
		} else if ( $lat && $lng ) {
			$query = $wpdb->prepare( "SELECT * FROM $table WHERE latitude = %s AND longitude = %s LIMIT 1", $lat, $lng );
		} else {
			return null;
		}
