	...
	final class PMXI_Plugin {
		...
		public function setup_allimport_dir() {
			...
			$wpallimportDirs = array(
				WP_ALL_IMPORT_UPLOADS_BASE_DIRECTORY,
				self::LOGS_DIRECTORY,
		...
		public function ver_4_transition_fix( &$options ) {
			...
			$exclude_taxonomies = ( class_exists( 'PMWI_Plugin' ) ) ? array(
				'post_format',
				'product_type',
			) : array( 'post_format' );
		...
		public function activation() {
			...
			if ( ! $wpai_options ) {
				update_option( $option_name, $options_default, false );
			}
		...
		public function fix_db_schema() {
			...
			if ( ! $iteration ) {
				...
				if ( empty( $grands ) ) {
					?>
					<div class="error"><p>
							<?php printf( __( '<b>%s Plugin</b>: Current sql user %s doesn\'t have ALTER privileges', 'wp-all-import-pro' ), self::getInstance()->getName(), DB_USER ) ?>
						</p></div>
					<?php
					return false;
				}
	...
	function wp_all_import_pro_updater() {
		if ( class_exists( 'PMXI_Updater' ) ) {
			...
			$updater = new PMXI_Updater( $wp_all_import_options['info_api_url_new'], __FILE__, array(
					...
					'license'   => ( ! empty( $wp_all_import_options['licenses']['PMXI_Plugin'] ) ) ? PMXI_Plugin::decode( $wp_all_import_options['licenses']['PMXI_Plugin'] ) : false,
					// license key (used get_option above to retrieve from DB)
					'item_name' => PMXI_Plugin::getEddName(),
					// name of this plugin
					'author'    => 'Soflyy',
