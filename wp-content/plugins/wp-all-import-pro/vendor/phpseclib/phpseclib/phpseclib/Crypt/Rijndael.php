...
class Rijndael extends BlockCipher {
	...
	protected function encryptBlock( $in ) {
		...
		$l = $c[3];
		while ( $i < $Nb ) {
			$temp[ $i ] = ( $state[ $i ] & intval( 0xFF000000 ) ) ^ ( $state[ $j ] & 0x00FF0000 ) ^ ( $state[ $k ] & 0x0000FF00 ) ^ ( $state[ $l ] & 0x000000FF ) ^ $w[ $i ];
			++ $i;
			$j = ( $j + 1 ) % $Nb;
			$k = ( $k + 1 ) % $Nb;
			$l = ( $l + 1 ) % $Nb;
	...
	protected function decryptBlock( $in ) {
		...
		while ( $i < $Nb ) {
			$word = ( $state[ $i ] & intval( 0xFF000000 ) ) | ( $state[ $j ] & 0x00FF0000 ) | ( $state[ $k ] & 0x0000FF00 ) | ( $state[ $l ] & 0x000000FF );
