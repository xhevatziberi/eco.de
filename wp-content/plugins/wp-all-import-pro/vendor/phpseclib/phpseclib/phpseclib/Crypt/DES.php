	...
	protected function setupKey() {
		...
		for ( $des_round = 0; $des_round < $this->des_rounds; ++ $des_round ) {
			...
			for ( $i = 0, $ki = 31; $i < 16; ++ $i, $ki -= 2 ) {
				...
				$val1                                           = ( $cp & intval( 0xFF000000 ) ) | ( ( $cp << 8 ) & 0x00FF0000 ) | ( ( $dp >> 16 ) & 0x0000FF00 ) | ( ( $dp >> 8 ) & 0x000000FF );
				$val2                                           = ( ( $cp << 8 ) & intval( 0xFF000000 ) ) | ( ( $cp << 16 ) & 0x00FF0000 ) | ( ( $dp >> 8 ) & 0x0000FF00 ) | ( $dp & 0x000000FF );
		...
		switch ( $this->des_rounds ) {
			case 3: // 3DES keys
				$this->keys = [
					self::ENCRYPT => array_merge( $keys[0][ self::ENCRYPT ], $keys[1][ self::DECRYPT ], $keys[2][ self::ENCRYPT ] ),
					self::DECRYPT => array_merge( $keys[2][ self::DECRYPT ], $keys[1][ self::ENCRYPT ], $keys[0][ self::DECRYPT ] ),
				];
				break;
			// case 1: // DES keys
			default:
