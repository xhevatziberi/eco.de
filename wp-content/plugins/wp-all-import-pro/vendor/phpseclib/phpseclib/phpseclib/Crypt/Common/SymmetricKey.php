...
abstract class SymmetricKey {
				...
				case ( php_uname( 'm' ) & "\xDF\xDF\xDF" ) == 'ARM':
					switch ( true ) {
						/* PHP 7.0.0 introduced a bug that affected 32-bit ARM processors:
                           ...
                           affected versions of PHP are: 7.0.x, 7.1.0 - 7.1.23 and 7.2.0 - 7.2.11 */ case PHP_VERSION_ID >= 70000 && PHP_VERSION_ID <= 70123:
	...
	protected function setup() {
		...
		if ( $this->usesNonce() ) {
			...
			if ( $this->mode == self::MODE_GCM && ! in_array( $this->engine, [
					self::ENGINE_LIBSODIUM,
					self::ENGINE_OPENSSL_GCM,
				] ) ) {
				$this->setupGCM();
