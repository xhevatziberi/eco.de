...
class SSH2 {
	...
	public function __construct( $host, $port = 22, $timeout = 10 ) {
		if ( empty( self::$message_numbers ) ) {
			...
			self::define_array( self::$message_numbers, self::$disconnect_reasons, self::$channel_open_failure_reasons, self::$terminal_modes, self::$channel_extended_data_type_codes, [ 60 => 'NET_SSH2_MSG_USERAUTH_PASSWD_CHANGEREQ' ], [ 60 => 'NET_SSH2_MSG_USERAUTH_PK_OK' ], [
	...
	private function key_exchange( $kexinit_payload_server = false ) {
		...
		switch ( true ) {
			...
			case substr( $this->server_identifier, 0, 13 ) == 'SSH-2.0-DLINK':
				if ( ! isset( $preferred['server_to_client']['mac'] ) ) {
					$s2c_mac_algorithms = array_values( array_diff( $s2c_mac_algorithms, [
							'hmac-sha1-96',
							'hmac-md5-96',
						] ) );
				}
				if ( ! isset( $preferred['client_to_server']['mac'] ) ) {
					$c2s_mac_algorithms = array_values( array_diff( $c2s_mac_algorithms, [
							'hmac-sha1-96',
							'hmac-md5-96',
						] ) );
				}
			...
			case substr( $this->server_identifier, 0, 24 ) == 'SSH-2.0-TurboFTP_SERVER_':
				if ( ! isset( $preferred['server_to_client']['crypt'] ) ) {
					$s2c_encryption_algorithms = array_values( array_diff( $s2c_encryption_algorithms, [
							'aes128-gcm@openssh.com',
							'aes256-gcm@openssh.com',
						] ) );
				}
				if ( ! isset( $preferred['client_to_server']['crypt'] ) ) {
					$c2s_encryption_algorithms = array_values( array_diff( $c2s_encryption_algorithms, [
							'aes128-gcm@openssh.com',
							'aes256-gcm@openssh.com',
						] ) );
				}
		...
		$kexinit_payload_client .= Strings::packSSH2( 'L10bN', $kex_algorithms, $server_host_key_algorithms, $c2s_encryption_algorithms, $s2c_encryption_algorithms, $c2s_mac_algorithms, $s2c_mac_algorithms, $c2s_compression_algorithms, $s2c_compression_algorithms, [], // language, client to server
	...
	private function keyboard_interactive_login( $username, $password ) {
		$packet = Strings::packSSH2( 'Cs5', NET_SSH2_MSG_USERAUTH_REQUEST, $username, 'ssh-connection', 'keyboard-interactive', '', // language tag
			'' // submethods
		);
		$this->send_binary_packet( $packet );
		...
		switch ( $type ) {
			case NET_SSH2_MSG_USERAUTH_INFO_REQUEST:
				list( , // name; may be empty
					...
					$num_prompts ) = Strings::unpackSSH2( 's3N', $response );
				...
				if ( isset( $this->keyboard_requests_responses ) ) {
					for ( $i = 0; $i < $num_prompts; $i ++ ) {
						list( $prompt, // prompt - ie. "Password: "; must not be empty
							// echo
		...
		switch ( $type ) {
			case NET_SSH2_MSG_USERAUTH_FAILURE:
				...
				if ( in_array( 'publickey', $auth_methods ) && substr( $signatureType, 0, 9 ) == 'rsa-sha2-' ) {
					$this->supported_private_key_algorithms = array_diff( $this->supported_private_key_algorithms, [
						'rsa-sha2-256',
						'rsa-sha2-512',
					] );
	...
	public function openShell() {
		...
		$terminal_modes = pack( 'C', NET_SSH2_TTY_OP_END );
		$packet         = Strings::packSSH2( 'CNsbsN4s', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[ self::CHANNEL_SHELL ], 'pty-req', true, // want reply
			$this->term, $this->windowColumns, $this->windowRows, 0, 0, $terminal_modes );
		...
		$packet = Strings::packSSH2( 'CNsb', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[ self::CHANNEL_SHELL ], 'shell', true // want reply
		);
		$this->send_binary_packet( $packet );
	...
	private function get_binary_packet() {
		...
		if ( $this->binary_packet_buffer == null ) {
			...
			$this->binary_packet_buffer = (object) [
				...
				// the total size of this packet to be read from the socket
				...
				case NET_SSH2_MSG_CHANNEL_OPEN: // see http://tools.ietf.org/html/rfc4254#section-5.1
					...
					switch ( $data ) {
						...
						case 'auth-agent@openssh.com':
							...
							break;
						default:
							$packet = Strings::packSSH2( 'CN2ss', NET_SSH2_MSG_CHANNEL_OPEN_FAILURE, $server_channel, NET_SSH2_OPEN_ADMINISTRATIVELY_PROHIBITED, '', // description
								'' // language tag
							);
							$this->send_binary_packet( $packet );
					}
						...
						switch ( $value ) {
							case 'exit-signal':
								list( , // FALSE
									...
									$error_message ) = Strings::unpackSSH2( 'bsbs', $response );
