...
class Sample {
	...
	public function write(
		Spreadsheet $spreadsheet, $filename, array $writers = [
		'Xlsx',
		'Xls',
	], bool $withCharts = false, ?callable $writerCallback = null
	): void {
	...
	public function logCalculationResult(
		Worksheet $worksheet, string $functionName, string $formulaCell, ?string $descriptionCell = null
	): void {
	...
	public function logWrite( IWriter $writer, $path, $callStartTime ): void {
		...
		$message = ( $this->isCli() === true ) ? "Write {$format} format to {$path}  in " . sprintf( '%.4f', $callTime ) . ' seconds' : "Write {$format} format to <code>{$path}</code>  in " . sprintf( '%.4f', $callTime ) . ' seconds';
