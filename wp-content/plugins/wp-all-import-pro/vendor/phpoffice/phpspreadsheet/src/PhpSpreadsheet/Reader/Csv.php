...
class Csv extends BaseReader {
	...
	public function castFormattedNumberToNumeric(
		bool $castFormattedNumberToNumeric, bool $preserveNumericFormatting = false
	): void {
	...
	private static function getCsv(
		$stream, ?int $length = null, string $separator = ',', string $enclosure = '"', ?string $escape = null
	) {
	...
	public static function affectedByPhp9(
		string $filename, string $inputEncoding = 'UTF-8', ?string $delimiter = null, string $enclosure = '"', string $escapeCharacter = '\\'
	): bool {
		...
		$reader1 = new self();
		$reader1->setInputEncoding( $inputEncoding )->setTestAutoDetect( true )->setEscapeCharacter( $escapeCharacter )->setDelimiter( $delimiter )->setEnclosure( $enclosure );
