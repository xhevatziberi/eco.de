...
class Xlsx extends BaseReader {
	...
	private function loadZip( string $filename, string $ns = '', bool $replaceUnclosedBr = false ): SimpleXMLElement {
		...
		$rels = @simplexml_load_string( $this->getSecurityScannerOrThrow()->scan( $contents ), 'SimpleXMLElement', 0, $ns );
	...
	private function loadZipNonamespace( string $filename, string $ns ): SimpleXMLElement {
		$contents = $this->getFromZipArchive( $this->zip, $filename );
		$rels     = simplexml_load_string( $this->getSecurityScannerOrThrow()->scan( $contents ), 'SimpleXMLElement', 0, ( $ns === '' ? $ns : '' ) );
			...
			if ( $mainNS !== '' ) {
				...
				if ( $xmlWorkbook->sheets ) {
					...
					foreach ( $xmlWorkbook->sheets->sheet as $eleSheet ) {
						...
						$xml = new XMLReader();
						$xml->xml( $this->getSecurityScannerOrThrow()->scan( $this->getFromZipArchive( $this->zip, $fileWorksheetPath ) ) );
		...
		foreach ( $rels->Relationship as $relx ) {
			...
			switch ( $relType ) {
				...
				case "$xmlNamespaceBase/officeDocument":
					...
					$sharedStrings = [];
					$relType       = "rel:Relationship[@Type='" //. Namespaces::SHARED_STRINGS
										...
										if ( isset( $shape['style'] ) ) {
											...
											if ( ( $column !== null ) && ( $row !== null ) ) {
												...
												if ( isset( $drowingImages[ $fillImageRelId ] ) ) {
													...
													$objDrawing->setPath( 'zip://' . File::realpath( $filename ) . '#' . $imagePath, true, $zip );
	...
	private function readRibbon( Spreadsheet $excel, string $customUITarget, ZipArchive $zip ): void {
		...
		if ( $dataRels ) {
			...
			$UIRels = simplexml_load_string( $this->getSecurityScannerOrThrow()->scan( $dataRels ) );
			if ( false !== $UIRels ) {
	...
	private function readAutoFilter(
		SimpleXMLElement $xmlSheet, Worksheet $docSheet
	): void {
	...
	private function readTables(
		SimpleXMLElement $xmlSheet, Worksheet $docSheet, string $dir, string $fileWorksheet, ZipArchive $zip
	): void {
	...
	private function readTablesInTablesFile(
		SimpleXMLElement $xmlSheet, string $dir, string $fileWorksheet, ZipArchive $zip, Worksheet $docSheet
	): void {
