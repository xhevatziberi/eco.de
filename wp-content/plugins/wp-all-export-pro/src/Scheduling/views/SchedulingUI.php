...
<div class="wpallexport-preview-content" style="max-height: 950px; overflow: visible;">
	...
	<div style="margin-bottom: 10px; margin-left:26px;">
		<label style="font-size: 13px;">
			<?php printf( /* translators: 1: Export ID */ esc_html__( 'Run export ID %d on a schedule.', 'wp_all_export_plugin' ), (int) $export_id ); ?>
		</label>
	</div>
				...
				<div class="input">
					<label style="color: rgb(68,68,68);">
						<input
							...
							name="scheduling_run_on" value="weekly" checked="checked" />
				...
				<div>
					<div class="input every-month">
						<label style="color: rgb(68,68,68); margin-top: 5px;">
							<input
								type="radio" <?php if ( isset( $post['scheduling_run_on'] ) && $post['scheduling_run_on'] == 'monthly' ) { ?> checked="checked" <?php } ?>
								name="scheduling_run_on" value="monthly" />
							<?php _e( 'Every month on the first...', 'wp_all_export_plugin' ); ?>
						</label>
					</div>
					<input type="hidden" name="scheduling_monthly_days"
					       value="<?php echo isset( $post['scheduling_monthly_days'] ) ? $post['scheduling_monthly_days'] : ''; ?>"
					       id="monthly_days" />
