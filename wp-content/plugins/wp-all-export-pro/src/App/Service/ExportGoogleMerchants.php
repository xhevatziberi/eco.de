...
class ExportGoogleMerchants {
	...
	public function export( $is_cron, $file_path, $exported_by_cron = 0 ) {
		...
		foreach ( $articles as $article ) {
			...
			$this->wordPressFilters->applyFilters( 'wp_all_export_after_csv_line', array(
				$stream,
	...
	private function prepareCsvHeaders( $headers, $stream, $exported = 0 ) {
		...
		$headers = $this->wordPressFilters->applyFilters( 'wp_all_export_csv_headers', array(
			$headers,
			XmlExportEngine::$exportID,
		) );

		if ( ! $exported ) {
			$this->fput_tsv( $stream, array_map( array(
				'XmlCsvExport',
				'_get_valid_header_name',
			), $headers ), self::GOOGLE_MERCHANTS_DELIMITER );
		}
	...
	private function exportRequestedData( $articles ) {
		while ( XmlExportEngine::$exportQuery->have_posts() ) {
			...
			$articles   = $this->wordPressFilters->applyFilters( 'wp_all_export_csv_rows', array(
				$articles,
