...
class FilteringOrders extends \Wpae\Pro\Filtering\FilteringCPT {
	...
	private function filterByProducts( $rule ) {
		...
		if ( ! empty( $mapping[ $rule->element ] ) ) {
			$rule->element = $mapping[ $rule->element ];
		}
		...
		$productsQuery = new \WP_Query( array( 'post_type'      => array( 'product', 'product_variation' ),
		                                       'post_status'    => 'any',
		                                       'orderby'        => 'ID',
		                                       'order'          => 'ASC',
		                                       'posts_per_page' => - 1,
	...
	public function posts_where( $where ) {
		if ( ! empty( $this->productsWhere ) ) {
			$where .= $this->productsWhere;
		}
