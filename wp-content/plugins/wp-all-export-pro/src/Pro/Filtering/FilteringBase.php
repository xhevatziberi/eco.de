...
abstract class FilteringBase implements FilteringInterface {
	...
	protected function parse_date_field( &$rule ) {
		...
		if ( strpos( $rule->value, "+" ) !== 0 && strpos( $rule->value, "-" ) !== 0 && strpos( $rule->value, 'first' ) !== 0 && strpos( $rule->value, 'last' ) !== 0 && strpos( $rule->value, "next" ) === false && strpos( $rule->value, "last" ) === false && ( strpos( $rule->value, "second" ) !== false || strpos( $rule->value, "minute" ) !== false || strpos( $rule->value, "hour" ) !== false || ( strpos( $rule->value, "day" ) !== false && strpos( $rule->value, "today" ) === false && strpos( $rule->value, "yesterday" ) === false ) || strpos( $rule->value, "week" ) !== false || strpos( $rule->value, "month" ) !== false || strpos( $rule->value, "year" ) !== false ) ) {
	...
	protected function recursion_parse_query( $parent_rule ) {
		...
		$sub_rules              = array();
		foreach ( $filter_rules_hierarchy as $j => $rule ) {
	...
	protected function parse_condition( $rule, $is_int = false, $table_alias = false ) {
		...
		switch ( $rule->condition ) {
			case 'equals':
				if ( in_array( $rule->element, array(
					'post_date',
					'comment_date',
					'user_registered',
					'user_role',
			...
			case 'is_empty':
				$q = " = ''";
				if ( $table_alias ) {
					$q .= " OR $table_alias.meta_value IS NULL";
				}
				break;
			case 'is_not_empty':
				$q = "IS NOT NULL ";
				if ( $table_alias ) {
					$q .= " AND $table_alias.meta_value <> '' ";
				}
				break;
			default:
				# code...
