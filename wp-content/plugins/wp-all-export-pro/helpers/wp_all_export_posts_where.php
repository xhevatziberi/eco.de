...
function wp_all_export_posts_where( $where ) {
	if ( ! empty( \PMXE_Plugin::$session ) && \PMXE_Plugin::$session->has_session() ) {
		...
		if ( ! empty( \XmlExportEngine::$exportOptions['whereclause'] ) ) {
			$where .= \XmlExportEngine::$exportOptions['whereclause'];
		}
	}
	...
	if ( isset( \XmlExportEngine::$exportOptions['enable_real_time_exports'] ) && \XmlExportEngine::$exportOptions['enable_real_time_exports'] ) {
		...
		// Real-Time Exports
		if ( ! empty( \XmlExportEngine::$exportOptions['whereclause'] ) && ! ( strpos( $where, \XmlExportEngine::$exportOptions['whereclause'] ) !== false ) ) {
			$where .= \XmlExportEngine::$exportOptions['whereclause'];
		}
