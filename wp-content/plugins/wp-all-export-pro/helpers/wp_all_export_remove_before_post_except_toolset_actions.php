		...
		foreach ( $wp_filter['pre_get_posts']->callbacks as $priority => $callback ) {
			foreach ( $callback as $callbackItem ) {
				if ( is_array( $callbackItem['function'] ) ) {
					if ( $callbackItem['function'][0] instanceof WPCF_Post_Types || $callbackItem['function'][0] instanceof WPToolset_Forms_Bootstrap || $callbackItem['function'][0] instanceof Toolset_Wp_Query_Adjustments_M2m ) {
