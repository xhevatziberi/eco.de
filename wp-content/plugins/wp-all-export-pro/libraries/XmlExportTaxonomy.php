...
if ( ! class_exists( 'XmlExportTaxonomy' ) ) {
	final class XmlExportTaxonomy {
		...
		public static function prepare_data( $term, $exportOptions, $xmlWriter, &$acfs, $implode_delimiter, $preview ) {
			...
			if ( ! empty( $xmlWriter ) and $exportOptions['export_to'] == 'xml' and ! in_array( $exportOptions['xml_template_type'], array(
					'custom',
					'XmlGoogleMerchants',
				) ) ) {
				$is_xml_export = true;
			...
			foreach ( $exportOptions['ids'] as $ID => $value ) {
				...
				if ( $exportOptions['cc_combine_multiple_fields'][ $ID ] ) {
					...
					if ( in_array( $fieldType, $addons ) ) {
						$article = apply_filters( "pmxe_{$fieldType}_addon_export_field", $article, $addonFieldOptions, $exportOptions, $ID, $term, $term->term_id, $xmlWriter, $element_name, $element_name_ns, $fieldSnipped, $preview );
					}
					...
					switch ( $fieldType ) {
						...
						case 'acf':
							...
							if ( ! empty( $fieldLabel ) and class_exists( 'acf' ) ) {
								...
								XmlExportACF::export_acf_field( $field_value, $exportOptions, $ID, $term->taxonomy . "_" . $term->term_id, $article, $xmlWriter, $acfs, $element_name, $element_name_ns, $fieldSnipped, $field_options['group_id'], $preview );
