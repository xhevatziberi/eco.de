...
if ( ! class_exists( 'XmlExportCustomRecord' ) ) {
	final class XmlExportCustomRecord {
		...
		public static function prepare_data( $record, $exportOptions, $xmlWriter, $implode_delimiter, $preview ) {
			...
			if ( $exportOptions['is_generate_import'] && wp_all_export_is_compatible() && ( ! isset( $exportOptions['enable_real_time_exports'] ) || ! $exportOptions['enable_real_time_exports'] ) && $exportOptions['import_id'] ) {
			...
			if ( ! empty( $xmlWriter ) && isset( $exportOptions['export_to'] ) && $exportOptions['export_to'] == 'xml' && ! in_array( $exportOptions['xml_template_type'], array(
					'custom',
					'XmlGoogleMerchants',
				) ) ) {
				$is_xml_export = true;
			...
			if ( $element_location === 'meta' ) {
				...
				if ( in_array( $element_name, $other_entry_data ) ) {
					...
					if ( in_array( $element_name, [
							...
							'read',
							'status',
						] ) || in_array( $element_name, [ 'Starred', 'Read', 'Status' ] ) ) {
