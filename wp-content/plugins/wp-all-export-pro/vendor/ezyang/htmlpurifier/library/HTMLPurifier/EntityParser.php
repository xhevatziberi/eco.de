...
class HTMLPurifier_EntityParser {
	...
	public function __construct() {
		...
		$this->_textEntitiesRegex = '/&(?:' . // hex
		                            '[#]x([a-fA-F0-9]+);?|' . // dec
		                            '[#]0*(\d+);?|' . // string (mandatory semicolon)
		                            // NB: order matters: match semicolon preferentially
		                            '([A-Za-z_:][A-Za-z0-9.\-_:]*);|' . // string (optional semicolon)
		                            "($semi_optional)" . ')/';

		$this->_attrEntitiesRegex = '/&(?:' . // hex
		                            '[#]x([a-fA-F0-9]+);?|' . // dec
		                            '[#]0*(\d+);?|' . // string (mandatory semicolon)
		                            // NB: order matters: match semicolon preferentially
		                            '([A-Za-z_:][A-Za-z0-9.\-_:]*);|' . // string (optional semicolon)
	...
	protected function entityCallback( $matches ) {
		...
		if ( $hex_part !== null && $hex_part !== "" ) {
			...
			if ( isset( $this->_entity_lookup->table[ $named_part ] ) ) {
				...
				if ( ! empty( $matches[3] ) ) {
					return preg_replace_callback( $this->_semiOptionalPrefixRegex, array(
							$this,
							'entityCallback',
						), $entity );
				}
