...
class Properties {
	...
	protected function readCustomProperties( SimpleXMLElement $xml, array $namespaces ): void {
		if ( isset( $xml->CustomDocumentProperties ) && is_iterable( $xml->CustomDocumentProperties[0] ) ) {
			...
			foreach ( $xml->CustomDocumentProperties[0] as $propertyName => $propertyValue ) {
				...
				$propertyName       = (string) preg_replace_callback( '/_x([0-9a-f]{4})_/i', [
					$this,
					'hex2str',
				], $propertyName );
	...
	protected function processStandardProperty(
		DocumentProperties $docProps, string $propertyName, string $stringValue
	): void {
		switch ( $propertyName ) {
			case 'Title':
	...
	protected function processCustomProperty(
		DocumentProperties $docProps, string $propertyName, ?SimpleXMLElement $propertyValue, SimpleXMLElement $propertyAttributes
	): void {
