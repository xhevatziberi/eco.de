...
function pmxe_admin_menu() {
	...
	if ( current_user_can( PMXE_Plugin::$capabilities ) ) { // admin management options
		...
		add_menu_page( __( 'WP All Export', 'wp_all_export_plugin' ), __( 'All Export', 'wp_all_export_plugin' ), PMXE_Plugin::$capabilities, 'pmxe-admin-home', array(
			PMXE_Plugin::getInstance(),
			'adminDispatcher',
		), 'data:image/svg+xml;base64,' . $icon_base64, 111 );
		// workaround to rename 1st option to `Home`
		$submenu['pmxe-admin-home'] = array();
		add_submenu_page( 'pmxe-admin-home', esc_html__( 'Export to XML', 'wp_all_export_plugin' ) . ' &lsaquo; ' . __( 'WP All Export', 'wp_all_export_plugin' ), __( 'New Export', 'wp_all_export_plugin' ), PMXE_Plugin::$capabilities, 'pmxe-admin-export', array(
			PMXE_Plugin::getInstance(),
			'adminDispatcher',
		) );
		add_submenu_page( 'pmxe-admin-home', esc_html__( 'Manage Exports', 'wp_all_export_plugin' ) . ' &lsaquo; ' . __( 'WP All Export', 'wp_all_export_plugin' ), __( 'Manage Exports', 'wp_all_export_plugin' ), PMXE_Plugin::$capabilities, 'pmxe-admin-manage', array(
			PMXE_Plugin::getInstance(),
			'adminDispatcher',
		) );
		add_submenu_page( 'pmxe-admin-home', esc_html__( 'Settings', 'wp_all_export_plugin' ) . ' &lsaquo; ' . __( 'WP All Export', 'wp_all_export_plugin' ), __( 'Settings', 'wp_all_export_plugin' ), PMXE_Plugin::$capabilities, 'pmxe-admin-settings', array(
			PMXE_Plugin::getInstance(),
			'adminDispatcher',
		) );
		...
		add_menu_page( __( 'WP All Export', 'wp_all_export_plugin' ), esc_html__( 'All Export', 'wp_all_export_plugin' ), PMXE_Plugin::CLIENT_MODE_CAP, 'pmxe-admin-manage', array(
			PMXE_Plugin::getInstance(),
			'adminDispatcher',
		), 'data:image/svg+xml;base64,' . $icon_base64, 111 );
	}
