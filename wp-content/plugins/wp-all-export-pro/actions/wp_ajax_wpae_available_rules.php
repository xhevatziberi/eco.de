...
function pmxe_wp_ajax_wpae_available_rules() {
	...
	<select id="wp_all_export_rule">
		...
		if ( strpos( $post['selected'], 'tx_' ) === 0 || strpos( $post['selected'], 'product_tx' ) === 0 ) {
			...
			<!-- Taxonomies -->
			<option
				value="in"><?php echo esc_html__( 'In', 'wp_all_export_plugin' ) . ' ' . esc_html( ucwords( str_replace( array(
						"product_tx",
						"tx_",
						"_",
					), array( "", "", " " ), $post['selected'] ) ) ); ?></option>
			<option
				value="not_in"><?php echo esc_html__( 'Not In', 'wp_all_export_plugin' ) . ' ' . esc_html( ucwords( str_replace( array(
						"product_tx",
						"tx_",
						"_",
					), array( "", "", " " ), $post['selected'] ) ) ); ?></option>
			...
			<?php
		} elseif ( in_array( $post['selected'], array(
				'post_date',
				'post_modified',
				'user_registered',
				'comment_date',
			...
			<option
				value="equals_or_greater"><?php esc_html_e( 'equal to or newer than', 'wp_all_export_plugin' ); ?></option>
			<option value="less"><?php esc_html_e( 'older than', 'wp_all_export_plugin' ); ?></option>
			<option
				value="equals_or_less"><?php esc_html_e( 'equal to or older than', 'wp_all_export_plugin' ); ?></option>
			...
			<option
				value="equals_or_greater"><?php esc_html_e( 'equal to or greater than', 'wp_all_export_plugin' ); ?></option>
			<option value="less"><?php esc_html_e( 'less than', 'wp_all_export_plugin' ); ?></option>
			<option
				value="equals_or_less"><?php esc_html_e( 'equal to or less than', 'wp_all_export_plugin' ); ?></option>
			<option value="is_empty"><?php esc_html_e( 'is empty', 'wp_all_export_plugin' ); ?></option>
			<option value="is_not_empty"><?php esc_html_e( 'is not empty', 'wp_all_export_plugin' ); ?></option>
			<?php
		} else {
