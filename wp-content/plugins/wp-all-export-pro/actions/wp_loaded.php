...
function pmxe_wp_loaded() {
	...
	if ( ! empty( $_GET['zapier_unsubscribe'] ) and ! empty( $_GET['api_key'] ) ) {
		...
		if ( ! empty( $zapier_api_key ) and $zapier_api_key == $_GET['api_key'] ) {
			...
			if ( ! empty( $subscriptions[ basename( $body['target_url'] ) ] ) ) {
				unset( $subscriptions[ basename( $body['target_url'] ) ] );
			}
	...
	if ( ! empty( $cron_job_key ) and ! empty( $_GET['export_id'] ) and ! empty( $_GET['export_key'] ) and $_GET['export_key'] == $cron_job_key and ! empty( $_GET['action'] ) and in_array( $_GET['action'], array(
			'processing',
			'trigger',
			'cancel',
		) ) ) {
		...
		if ( ! empty( $ids ) and is_array( $ids ) ) {
			foreach ( $ids as $id ) {
				...
				if ( ( ! $addons->isWooCommerceAddonActive() && ! $addons->isWooCommerceProductAddonActive() && strpos( $export->options['wp_query'], 'product' ) !== false && \class_exists( 'WooCommerce' ) ) || ( ! $addons->isWooCommerceAddonActive() && ! $addons->isWooCommerceOrderAddonActive() && strpos( $export->options['wp_query'], 'shop_order' ) !== false ) || ( ! $addons->isWooCommerceAddonActive() && strpos( $export->options['wp_query'], 'shop_coupon' ) !== false )
				...
				if ( ( ( in_array( 'product', $cpt ) && in_array( 'product_variation', $cpt ) ) || ( in_array( 'shop_order', $cpt ) && ! $addons->isWooCommerceOrderAddonActive() ) || in_array( 'shop_coupon', $cpt ) || in_array( 'shop_review', $cpt ) ) && ! $addons->isWooCommerceAddonActive() ) {
			...
			if ( ! $export->isEmpty() ) {
				switch ( $_GET['action'] ) {
					case 'get_data':
						...
						if ( ! empty( $_GET['part'] ) and is_numeric( $_GET['part'] ) ) {
							$filepath = str_replace( basename( $filepath ), str_replace( '.' . $export->options['export_to'], '', basename( $filepath ) ) . '-' . $_GET['part'] . '.' . $export->options['export_to'], $filepath );
						}
