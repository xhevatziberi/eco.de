...
abstract class PMXE_Addon_Base {
	...
	public function getGroupsByExportType() {
		return static::groups( \XmlExportEngine::$post_types, \XmlExportEngine::$exportOptions['taxonomy_to_export'] );
	}

	public function getFieldsByExportType() {
		$fields = static::fields( \XmlExportEngine::$post_types, \XmlExportEngine::$exportOptions['taxonomy_to_export'] );

		return array_map( fn( $field ) => array_merge( $field, [
			'addon'     => $this->slug,
			'subfields' => array_map( fn( $subfield ) => array_merge( $subfield, [
				'addon' => $this->slug,
			] ), $field['subfields'] ?? [] ),
		] ), $fields );
	}
	...
	public function getFieldsByGroup( $group_id ) {
		...
		return array_values( array_filter( $fields, function ( $field ) use ( $group_id ) {
...
trait HasError {
	...
	public function getMissingDependencyError( $pluginName, $pluginUrl ) {
		return new \WP_Error( 'missing_dependency', __( sprintf( "<b>%s Export Add-on Plugin</b>: <a target=\"_blank\" href=\"%s\">%s</a> must be installed", $this->name(), $pluginUrl, $pluginName ), 'wp_all_import_plugin' ) );
	}
}
