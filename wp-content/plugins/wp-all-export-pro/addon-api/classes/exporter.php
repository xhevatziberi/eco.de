...
class PMXE_Addon_Exporter {
	...
	public static function isXmlExport( $xmlWriter ) {
		...
		return ! empty( $xmlWriter ) && $options['export_to'] == 'xml' && ! in_array( $options['xml_template_type'], [
				'custom',
				'XmlGoogleMerchants',
			] );
	}
	 ...
	 */
	public function run(
		$article, $fieldData, $exportOptions, $ID, $entry, $entryID, $xmlWriter, $elementName, $elementNameNs, $phpFunction, $preview
	) {
		$settings = $this->getFieldSettings( $exportOptions, $ID );
		$resolver = new PMXE_Addon_Resolver( $this->addon, $entry, $entryID );

		$field = PMXE_Addon_Field::from( $fieldData, $resolver, $settings, $elementName, $elementNameNs, $phpFunction );
	...
	public function getHeaders(
		$headers, $fieldData, $exportOptions, $entryID, $elementName
	) {
		$settings = $this->getFieldSettings( $exportOptions, $entryID );
		$resolver = new PMXE_Addon_Resolver( $this->addon, null, null );

		$field = PMXE_Addon_Field::from( $fieldData, $resolver, $settings, $elementName );
