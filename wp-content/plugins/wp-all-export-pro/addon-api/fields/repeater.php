...
class PMXE_Addon_Repeater_Field extends PMXE_Addon_Field {
	...
	public function getSubField( $key, $index ) {
		foreach ( $this->subfields as $subfield ) {
			if ( $subfield['key'] === $key ) {
				return PMXE_Addon_Field::from( $subfield, $this->resolver, $this->settings, $this->elName . '_' . $key, $this->elNameNs, $this->phpFunction, $this, $index );
			}
		}
	...
	public function appendNewArticles( $baseArticle ) {
		...
		foreach ( $rows as $rowIndex => $row ) {
			...
			$articles[] = [
				'content'  => $this->shouldFillEmptyColumns() ? array_merge( $baseArticle, $formatted_row ) : $formatted_row,
