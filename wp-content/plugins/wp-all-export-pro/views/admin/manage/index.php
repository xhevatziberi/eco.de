	...
	<div class="tablenav">
		<div class="alignleft actions">
			...
			<select name="bulk-action">
				...
				<option
					value="allow_client_mode"><?php esc_html_e( 'Toggle Client Mode', 'wp_all_export_plugin' ) ?></option>
			</select>
		...
		<?php if ( $page_links ): ?>
			<div class="tablenav-pages">
				<?php echo $page_links_html = sprintf( '<span class="displaying-num">' . esc_html__( 'Displaying %s&#8211;%s of %s', 'wp_all_export_plugin' ) . '</span>%s', number_format_i18n( ( intval( $pagenum ) - 1 ) * intval( $perPage ) + 1 ), number_format_i18n( min( intval( $pagenum * $perPage ), intval( $list->total() ) ) ), number_format_i18n( $list->total() ), $page_links ) ?>
			</div>
		<?php endif ?>
	</div>
	<div class="clear"></div>
						...
						switch ( $column_id ):
							...
							case 'name':
								...
								<td style="min-width: 300px;">
									<strong><?php echo wp_all_export_clear_xss( $item['friendly_name'] ); ?></strong>
									<br>
									<div class="row-actions">
										<?php if ( current_user_can( PMXE_Plugin::$capabilities ) ) { ?>
											<span class="edit"><a class="edit"
											                      ...
											                      ), $this->baseUrl ) ) ?>"><?php esc_html_e( 'Edit Template', 'wp_all_export_plugin' ) ?></a></span> |
											<span class="edit"><a class="edit"
											                      href="<?php echo esc_url( add_query_arg( array(
												                      'id'               => $item['id'],
												                      'action'           => 'options',
												                      '_wpnonce_options' => wp_create_nonce( 'options' ),
											                      ), $this->baseUrl ) ) ?>"><?php esc_html_e( 'Settings', 'wp_all_export_plugin' ) ?></a></span>
										...
										<?php if ( ! $is_secure_import and $item['attch_id'] ): ?>
											| <span class="update"> <a class="update"
											                           ...
											                           ), $this->baseUrl ) ) ?>"><?php echo esc_html( strtoupper( wp_all_export_get_export_format( $item['options'] ) ) ); ?></a></span>
											<?php if ( ! empty( $item['options']['bundlepath'] ) and PMXE_Export_Record::is_bundle_supported( $item['options'] ) ): ?>
												| <span class="update"><a class="update"
												                          ...
												                          ), $this->baseUrl ) ) ?>"><?php esc_html_e( 'Bundle', 'wp_all_export_plugin' ); ?></a></span>
											<?php endif; ?>
										<?php endif; ?>
											...
											<?php if ( $is_secure_import and ! empty( $item['options']['filepath'] ) ): ?>
												| <span class="update"><a class="update"
												                          href="<?php echo esc_url( add_query_arg( array(
													                          ...
													                          'action'   => 'get_file',
													                          '_wpnonce' => wp_create_nonce( '_wpnonce-download_feed' ),
												                          ), $this->baseUrl ) ) ?>"><?php if ( current_user_can( PMXE_Plugin::CLIENT_MODE_CAP ) && ! current_user_can( PMXE_Plugin::$capabilities ) ) { ?>Download <?php } ?>
                                                        TXT</a></span>
											<?php endif; ?>
										<?php else: ?>
											<?php if ( $is_secure_import && ! empty( $item['options']['filepath'] ) ): ?>
												| <span class="update"><a class="update"
												                          href="<?php echo esc_url( add_query_arg( array(
													                          ...
													                          'action'   => 'get_file',
													                          '_wpnonce' => wp_create_nonce( '_wpnonce-download_feed' ),
												                          ), $this->baseUrl ) ) ?>"><?php if ( current_user_can( PMXE_Plugin::CLIENT_MODE_CAP ) && ! current_user_can( PMXE_Plugin::$capabilities ) ) { ?>Download <?php } ?><?php echo strtoupper( wp_all_export_get_export_format( $item['options'] ) ); ?></a></span>
										...
										<?php if ( ! empty( $item['options']['split_large_exports'] ) and ! empty( $item['options']['split_files_list'] ) and current_user_can( PMXE_Plugin::$capabilities ) ): ?>
											| <span class="update"><a class="update"
											                          ...
											                          ), $this->baseUrl ) ) ?>"><?php printf( esc_html__( 'Split %ss', 'wp_all_export_plugin' ), strtoupper( wp_all_export_get_export_format( $item['options'] ) ) ); ?></a></span> |
										<?php endif; ?>
										...
										<?php if ( current_user_can( PMXE_Plugin::$capabilities ) ) { ?>
											| <span class="delete"><a class="delete"
											                          href="<?php echo esc_url( add_query_arg( array(
												                          'id'     => $item['id'],
												                          'action' => 'delete',
											                          ), $this->baseUrl ) ) ?>"><?php esc_html_e( 'Delete', 'wp_all_export_plugin' ) ?></a></span>
										<?php } ?>
									</div>
								...
								<td style="min-width: 180px;max-width: 180px;">
									<?php if ( current_user_can( PMXE_Plugin::$capabilities ) && ! is_real_time_exports( $item ) ) { ?>
										<a
											...
											if ( ( ( in_array( 'users', $item['options']['cpt'] ) || in_array( 'shop_customer', $item['options']['cpt'] ) ) && ! $addons->isUserAddonActive() ) || ( $item['options']['export_type'] == 'advanced' && $item['options']['wp_query_selector'] == 'wp_user_query' && ! $addons->isUserAddonActive() ) ) {
												?>
												href="<?php echo esc_url( add_query_arg( array(
													'id'               => $item['id'],
													'action'           => 'options',
											...
											} else if ( ( ( ( in_array( 'product', $item['options']['cpt'] ) && in_array( 'product_variation', $item['options']['cpt'] ) && ! $addons->isWooCommerceProductAddonActive() ) || ( in_array( 'shop_order', $item['options']['cpt'] ) && ! $addons->isWooCommerceOrderAddonActive() ) || in_array( 'shop_coupon', $item['options']['cpt'] ) || in_array( 'shop_review', $item['options']['cpt'] ) || ( isset( $item['options']['xml_template_type'] ) && $item['options']['xml_template_type'] == \XmlExportEngine::EXPORT_TYPE_GOOLE_MERCHANTS ) ) && ! $addons->isWooCommerceAddonActive() ) || ( $item['options']['export_type'] == 'advanced' && isset( $item['options']['exportquery'] ) && in_array( $item['options']['exportquery']->query['post_type'], array( 'shop_coupon' ) ) && ! $addons->isWooCommerceAddonActive() ) || ( $item['options']['export_type'] == 'advanced' && isset( $item['options']['exportquery'] ) && in_array( $item['options']['exportquery']->query['post_type'], array( 'shop_order' ) ) && ! $addons->isWooCommerceAddonActive() && ! $addons->isWooCommerceOrderAddonActive() ) || ( $item['options']['export_type'] == 'advanced' && isset( $item['options']['exportquery'] ) && in_array( $item['options']['exportquery']->query['post_type'], array(
														array(
															'product',
															'product_variation',
														),
											...
											} else if ( ( ( ! in_array( 'comments', $item['options']['cpt'] ) || ! in_array( 'shop_review', $item['options']['cpt'] ) ) && in_array( 'acf', $item['options']['cc_type'] ) && ! $addons->isAcfAddonActive() ) || ( $item['options']['export_type'] == 'advanced' && $item['options']['wp_query_selector'] != 'wp_comment_query' && in_array( 'acf', $item['options']['cc_type'] ) && ! $addons->isAcfAddonActive() ) ) {
												?>
												href="<?php echo esc_url( add_query_arg( array(
													'id'               => $item['id'],
													'action'           => 'options',
									...
									<?php if ( $item['options']['export_to'] === XmlExportEngine::EXPORT_TYPE_XML && $item['options']['xml_template_type'] == XmlExportEngine::EXPORT_TYPE_GOOLE_MERCHANTS ) : ?>
										<a href="<?php echo esc_url_raw( add_query_arg( array(
											'id'     => $item['id'],
											'action' => 'google_merchants_info',
										), $this->baseUrl ) ); ?>"><?php esc_html_e( 'Google Merchant Center Info', 'wp_all_export_plugin' ); ?></a>
									<?php endif; ?>
									<?php
									...
									if ( ! is_real_time_exports( $item ) ) {
										...
										if ( $item['options']['export_to'] == 'csv' || ( empty( $item['options']['xml_template_type'] ) || ! in_array( $item['options']['xml_template_type'], array(
														'custom',
														'XmlGoogleMerchants',
													) ) ) ): ?>
											<?php if ( wp_all_export_is_compatible() and ! empty( $item['options']['import_id'] ) and $is_re_import_allowed ): ?>
												<a href="<?php echo esc_url_raw( add_query_arg( array(
													...
													'id'       => $item['options']['import_id'],
													'deligate' => 'wpallexport',
												), remove_query_arg( 'page', $this->baseUrl ) ) ); ?>"><?php esc_html_e( "Import with WP All Import", "wp_all_export_plugin" ); ?></a>
												<br />
											<?php endif; ?>
											<?php
											if ( ! in_array( $item['options']['wp_query_selector'], array( 'wp_comment_query' ) ) and ( empty( $item['options']['cpt'] ) or ! in_array( 'comments', $item['options']['cpt'] ) ) ) {
												if ( ! empty( $item['options']['tpl_data'] ) ) {
													...
													<a href="<?php echo esc_url_raw( add_query_arg( array(
														'id'     => $item['id'],
														'action' => 'templates',
													), $this->baseUrl ) ); ?>"><?php esc_html_e( 'Download Import Templates', 'wp_all_export_plugin' ); ?></a>
													<?php
												}
											}
											?>
											...
											<?php if ( ! $item['processing'] and ! $item['executing'] ): ?>
												<h2 style="float:left;"><a class="add-new-h2"
												                           href="<?php echo esc_url_raw( add_query_arg( array(
													                           'id'     => $item['id'],
													                           'action' => 'update',
												                           ), $this->baseUrl ) ); ?>"><?php esc_html_e( 'Run Export', 'wp_all_export_plugin' ); ?></a>
												</h2>
											<?php elseif ( $item['processing'] ) : ?>
												<h2 style="float:left;"><a class="add-new-h2"
												                           href="<?php echo esc_url_raw( add_query_arg( array(
													                           'id'     => $item['id'],
													                           'action' => 'cancel',
												                           ), $this->baseUrl ) ); ?>"><?php esc_html_e( 'Cancel Cron', 'wp_all_export_plugin' ); ?></a>
												</h2>
											<?php elseif ( $item['executing'] ) : ?>
												<h2 style="float:left;"><a class="add-new-h2"
												                           href="<?php echo esc_url_raw( add_query_arg( array(
													                           'id'     => $item['id'],
													                           'action' => 'cancel',
												                           ), $this->baseUrl ) ); ?>"><?php esc_html_e( 'Cancel', 'wp_all_export_plugin' ); ?></a>
												</h2>
											<?php endif; ?>
										<?php else: ?>
			...
			<?php if ( current_user_can( PMXE_Plugin::$capabilities ) ) { ?>
				...
				<select name="bulk-action2">
					...
					<?php else: ?>
						...
						<option
							value="delete"><?php esc_html_e( 'Delete Permanently', 'wp_all_export_plugin' ) ?></option>
					<?php endif ?>
				</select>
