...
<?php if ( ! $is_license_active ): ?>
	<form name="settings" method="post" action="" class="settings">
		...
		<div class="wpallexport-setting-wrapper">
			...
			<table class="form-table">
				...
				<tr>
					<th scope="row">
						<label><?php esc_html_e( 'WP All Export License Key', 'wp_all_export_plugin' ); ?></label></th>
					<td>
						<input type="password" class="regular-text" name="license"
						       value="<?php if ( ! empty( $post['license'] ) ) {
							       esc_attr_e( PMXE_Plugin::decode( $post['license'] ) );
						       } ?>" />
						<?php if ( ! empty( $post['license'] ) ) { ?>

							<?php if ( ! empty( $post['license_status'] ) && $post['license_status'] == 'valid' ) { ?>
								<div
									class="license-status inline updated"><?php esc_html_e( 'Active', 'wp_all_export_plugin' ); ?></div>
							<?php } else { ?>
								<?php if ( ! empty( $_POST['license'] ) ) { ?>
									<div
										class="license-status inline error"><?php echo $post['license_status']; ?></div>
								<?php } ?>
							<?php } ?>
	...
	<form name="settings" method="post" action="" class="settings">
		...
		<table class="form-table">
			...
			<tr>
				<th scope="row">
					<label><?php esc_html_e( 'Automatic Scheduling License Key', 'wp_all_export_plugin' ); ?></label>
				</th>
				<td>
					<input type="password" class="regular-text" name="scheduling_license"
					       value="<?php if ( ! empty( $post['scheduling_license'] ) ) {
						       esc_attr_e( PMXE_Plugin::decode( $post['scheduling_license'] ) );
					       } ?>" />
					<?php if ( ! empty( $post['scheduling_license'] ) ) { ?>
						...
						<?php } else { ?>
							...
							<?php if ( ! empty( $_POST['scheduling_license'] ) ) { ?>
								<div
									class="license-status inline error"><?php echo esc_html( $post['scheduling_license_status'] ); ?></div>
					...
					<tr>
						<td>
							<label class="selectit" for="template-<?php echo esc_attr( $t->id ) ?>"><input
									...
									value="<?php echo esc_attr( $t->id ) ?>" /> <?php echo esc_html( wp_all_export_clear_xss( $t->name ) ); ?>
							</label>
						</td>
					</tr>
				<?php endforeach ?>
			...
			<td>
				<fieldset style="padding:0;">
					<legend class="screen-reader-text">
						<span><?php esc_html_e( 'Secure Mode', 'wp_all_export_plugin' ); ?></span>
					</legend>
		...
		<tr>
			...
			<td>
				<input type="text" class="regular-text" name="zapier_api_key" readOnly="readOnly"
				       value="<?php if ( ! empty( $post['zapier_api_key'] ) ) {
					       esc_attr_e( $post['zapier_api_key'] );
				       } ?>" />
...
<?php if ( $is_license_active ): ?>
	...
	<form name="settings" method="post" action="" class="settings">
		...
		<table class="form-table">
			...
			<tr>
				<th scope="row"><label><?php esc_html_e( 'Scheduling License Key', 'wp_all_export_plugin' ); ?></label>
				</th>
				<td>
					<input type="password" class="regular-text" name="scheduling_license"
					       value="<?php if ( ! empty( $post['scheduling_license'] ) ) {
