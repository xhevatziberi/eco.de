...
<div ng-app="GoogleMerchants"
     ...
     ng-init="init('<?php if ( class_exists( "WooCommerce" ) ) {
	     echo esc_attr( get_woocommerce_currency_symbol() );
     } ?>',
     '<?php if ( class_exists( "WooCommerce" ) ) {
	     echo esc_attr( get_woocommerce_currency() );
     } ?>',
     <?php $is_template_loaded = PMXE_Plugin::$session->get( 'is_loaded_template' );
     if ( ! empty( $is_template_loaded ) ) {
	     echo esc_html( PMXE_Plugin::$session->get( 'is_loaded_template' ) );
     } else {
	     echo "false";
     } ?>)"
