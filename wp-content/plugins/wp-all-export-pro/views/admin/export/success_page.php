			...
			<div class="tab-content selected normal-tab" id="tab1-content">
				...
				<div class="input">
					<button class="button button-primary button-hero wpallexport-large-button download_data"
					        rel="<?php echo esc_url_raw( add_query_arg( array( 'page'     => 'pmxe-admin-manage',
					                                                           'action'   => 'download',
					                                                           'id'       => $update_previous->id,
					                                                           '_wpnonce' => wp_create_nonce( '_wpnonce-download_feed' ),
					        ), $this->baseUrl ) ); ?>"><?php echo strtoupper( wp_all_export_get_export_format( $update_previous->options ) ); ?></button>
					...
					<?php if ( PMXE_Export_Record::is_bundle_supported( $update_previous->options ) ): ?>
						<button class="button button-primary button-hero wpallexport-large-button download_data"
						        ...
						        ), $this->baseUrl ) ); ?>"><?php esc_html_e( 'Bundle', 'wp_all_export_plugin' ); ?></button>
					<?php endif; ?>
				</div>
				...
				<div style="margin-top:30px;">
					...
					<div id="wpae-secure-url-container">
						...
						<button class="button button-hero" id="wpae-generate-token"
						        data-id="<?php echo $update_previous->id; ?>">
							<img style="display: none;"
							     src="<?php echo esc_attr( PMXE_ROOT_URL ); ?>/static/img/preloader.gif" />
						...
						<input placeholder="Click generate to generate a secure URL" type="text" id="wpae-secure-url"
						       style=""
						       value="<?php
					...
					<?php if ( $update_previous->options['enable_real_time_exports'] ) { ?>
						<p>
							Each time a new post is created, the export will run and a new CSV file is created. The
							secure URL will point to that CSV file.

						</p>
						<p style="margin-bottom: 10px; font-size: 12px; line-height: 22px;">This export will run every
							time a new <strong><?php echo esc_html( $cpt_name ); ?></strong>
				...
				<div class="wrap" style="text-align: left; padding-top: 10px;">
					<div id="scheduling-form-container">
						...
						<div class="wpallexport-content-section"
						     style="padding-bottom: 15px; margin-bottom: 10px; margin-top: 5px;">
							<div class="wpallexport-collapsed-content" style="padding: 0; height: auto; ">
								<div class="wpallexport-collapsed-content-inner"
								     style="padding-bottom: 0; overflow: auto; padding-right: 0;">
					...
					<div
						class="wpae-save-button button button-primary button-hero wpallexport-large-button wpae-export-complete-save-button <?php if ( ! $hasActiveLicense ) {
							echo 'disabled';
						} ?>"
		...
		<p>
			<button class="button button-primary button-hero wpallexport-large-button download_data"
			        rel="<?php echo esc_url_raw( add_query_arg( array( 'page'     => 'pmxe-admin-manage',
			                                                           ...
			                                                           'id'       => $update_previous->id,
			                                                           '_wpnonce' => wp_create_nonce( '_wpnonce-download_feed' ),
			        ), $this->baseUrl ) ); ?>"><?php esc_html_e( 'Download', 'wp_all_export_plugin' ); ?><?php echo esc_html( strtoupper( wp_all_export_get_export_format( $update_previous->options ) ) ); ?></button>

			<button class="button button-primary button-hero wpallexport-large-button download_data"
			        rel="<?php echo esc_url_raw( add_query_arg( array( 'page'     => 'pmxi-admin-import',
			                                                           'id'       => $update_previous->options['import_id'],
			                                                           'deligate' => 'wpallexport',
			        ), remove_query_arg( 'page', $this->baseUrl ) ) ); ?>"><?php esc_html_e( 'Import with WP All Import', 'wp_all_export_plugin' ); ?></button>
		</p>
		<p>
			...
			<?php if ( XmlExportEngine::$exportOptions['export_to'] == XmlExportEngine::EXPORT_TYPE_XML && XmlExportEngine::$exportOptions['xml_template_type'] == XmlExportEngine::EXPORT_TYPE_GOOLE_MERCHANTS ) { ?>
				<button class="button button-primary button-hero wpallexport-large-button download_data"
				        rel="<?php echo esc_url_raw( add_query_arg( array( 'page'     => 'pmxe-admin-manage',
				                                                           ...
				                                                           'id'       => $update_previous->id,
				                                                           '_wpnonce' => wp_create_nonce( '_wpnonce-download_feed' ),
				        ), $this->baseUrl ) ); ?>"><?php esc_html_e( 'Download ', 'wp_all_export_plugin' ); ?>
					TXT
				</button>

			<?php } else { ?>
				<button class="button button-primary button-hero wpallexport-large-button download_data"
				        rel="<?php echo esc_url_raw( add_query_arg( array( 'action'   => 'download',
				                                                           'id'       => $update_previous->id,
				                                                           '_wpnonce' => wp_create_nonce( '_wpnonce-download_feed' ),
				        ), $this->baseUrl ) ); ?>"><?php esc_html_e( 'Download ', 'wp_all_export_plugin' ); ?><?php echo esc_html( strtoupper( wp_all_export_get_export_format( $update_previous->options ) ) ); ?></button>
