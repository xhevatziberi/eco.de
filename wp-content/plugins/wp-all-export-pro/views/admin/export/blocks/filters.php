...
<div class="wp_all_export_rule_inputs">
	<table>
		...
		<tr>
			...
			<td style="width: 15%;">
				<a id="wp_all_export_add_rule"
				   href="javascript:void(0);"><?php esc_html_e( 'Add Rule', 'wp_all_export_plugin' ); ?></a>
			</td>
		</tr>
	</table>
</div>
<div id="wpallexport-filters" style="padding:0;">
	<div class="wpallexport-content-section" style="padding:0; border: none;">
		<fieldset id="wp_all_export_filtering_rules">
			...
			?>
			<p id="date_field_notice"
			   style="margin: 5px 0px 20px; text-align: center;"><?php echo wp_kses_post( __( 'Date filters use natural language.<br>For example, to return records created in the last week: <i>date ▸ newer than ▸ last week</i>.<br>For all records created in 2016: <i>date ▸ older than ▸ 1/1/2017</i> AND <i>date ▸ newer than ▸ 12/31/2015</i>', 'wp_all_export_plugin' ) ); ?>
				.</p>
			<p id="no_options_notice"
			   style="margin:20px 0 5px; text-align:center; <?php if ( ! empty( $filter_rules_hierarhy ) and is_array( $filter_rules_hierarhy ) ) {
				   echo 'display:none;';
			   } ?>"><?php esc_html_e( 'No filtering options. Add filtering options to only export records matching some specified criteria.', 'wp_all_export_plugin' ); ?></p>
						...
						if ( is_null( $rule->parent_id ) ) {
							...
							<li id="item_<?php echo intval( $rulenumber ); ?>" class="dragging">
								<div class="drag-element">
									...
									<input type="hidden" value="<?php echo esc_attr( $rule->title ); ?>"
									       class="wp_all_export_xml_element_title"
									       name="wp_all_export_xml_element_title[<?php echo intval( $rulenumber ); ?>]" />
									...
									<span
										class="condition <?php if ( $rulenumber == count( $filter_rules_hierarhy ) ) : ?>last_condition<?php endif; ?>">
											...
											<input id="rule_and_<?php echo intval( $rulenumber ); ?>" type="radio"
											       value="and" name="rule[<?php echo intval( $rulenumber ); ?>]"
											...
											<input id="rule_or_<?php echo intval( $rulenumber ); ?>" type="radio"
											       value="or" name="rule[<?php echo intval( $rulenumber ); ?>]"
